...
## 🚫 CRITICAL: FILE CREATION RESTRICTIONS (MANDATORY)

### ❌ NEVER CREATE FILES IN THESE LOCATIONS:
```
❌ / (project root) - NO .md, .sql, .py, .ts, .json files
❌ /docs/ - Reserved for PUBLIC developer documentation only
❌ /scripts/ root - Use /scripts/codegen/, /scripts/build/, etc.
❌ /apps/ root - Use proper feature directories
❌ /services/ root - Use /services/ai-engine/src/ subdirectories
❌ Any random location not in the approved structure
```
...
1. ✅ **CHECK** if it belongs in `/.claude/docs/` (internal) or `/docs/` (public)
2. ✅ **VERIFY** the correct subdirectory exists
3. ✅ **SEARCH** for existing similar files first
4. ✅ **ASK** if unsure about the correct location
5. ❌ **NEVER** create temporary files in project root
6. ❌ **NEVER** create documentation outside designated folders

### Documentation Location Rules:

```
/.claude/docs/                    # INTERNAL comprehensive docs
├── architecture/                 # Architecture decisions & standards
├── services/                     # Service-specific PRDs & ARDs
│   ├── ask-expert/              # Ask Expert documentation
│   └── ask-panel/               # Ask Panel documentation
├── platform/                     # Platform features (agents, personas)
