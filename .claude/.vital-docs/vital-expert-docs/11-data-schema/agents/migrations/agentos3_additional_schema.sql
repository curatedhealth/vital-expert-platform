-- ==========================================
-- FILE: agentos3_additional_schema.sql
-- PURPOSE: Additional schema objects required for AgentOS 3.0 implementation
-- PHASE: AgentOS 3.0 Foundation
-- DEPENDENCIES: AgentOS 2.0 (34 tables, 6 views) must be complete
-- GOLDEN RULES: Evidence-based tiering, comprehensive monitoring, safety-first
-- ==========================================

DO $$
BEGIN
    RAISE NOTICE '=================================================================';
    RAISE NOTICE 'AGENTOS 3.0: ADDITIONAL SCHEMA OBJECTS';
    RAISE NOTICE '=================================================================';
    RAISE NOTICE '';
END $$;

-- ==========================================
-- SECTION 1: AGENT KG VIEWS
-- ==========================================

DO $$
BEGIN
    RAISE NOTICE '--- Section 1: Creating Agent KG Views Table ---';
END $$;
...
CREATE INDEX IF NOT EXISTS idx_agent_kg_views_agent ON agent_kg_views(agent_id);
CREATE INDEX IF NOT EXISTS idx_agent_kg_views_tenant ON agent_kg_views(tenant_id);
CREATE INDEX IF NOT EXISTS idx_agent_kg_views_active ON agent_kg_views(is_active);

COMMENT ON TABLE agent_kg_views IS 'Agent-specific knowledge graph access control and traversal limits';

DO $$
BEGIN
    RAISE NOTICE '✓ Created agent_kg_views table';
END $$;

-- ==========================================
-- SECTION 2: INTERACTION LOGGING
-- ==========================================

DO $$
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '--- Section 2: Creating Agent Interaction Logs Table ---';
END $$;
...
CREATE INDEX IF NOT EXISTS idx_agent_interaction_logs_session ON agent_interaction_logs(session_id);
CREATE INDEX IF NOT EXISTS idx_agent_interaction_logs_agent ON agent_interaction_logs(agent_id);
CREATE INDEX IF NOT EXISTS idx_agent_interaction_logs_user ON agent_interaction_logs(user_id);
CREATE INDEX IF NOT EXISTS idx_agent_interaction_logs_tenant ON agent_interaction_logs(tenant_id);
CREATE INDEX IF NOT EXISTS idx_agent_interaction_logs_tier ON agent_interaction_logs(tier_level);
CREATE INDEX IF NOT EXISTS idx_agent_interaction_logs_created ON agent_interaction_logs(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_agent_interaction_logs_escalated ON agent_interaction_logs(escalated);
CREATE INDEX IF NOT EXISTS idx_agent_interaction_logs_domain ON agent_interaction_logs(domain);

COMMENT ON TABLE agent_interaction_logs IS 'Comprehensive logging of all agent interactions for monitoring and analytics';

DO $$
BEGIN
    RAISE NOTICE '✓ Created agent_interaction_logs table with 8 indexes';
END $$;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '--- Section 3: Creating Agent Tiers Table ---';
END $$;
...
CREATE INDEX IF NOT EXISTS idx_agent_tiers_level ON agent_tiers(level);
CREATE INDEX IF NOT EXISTS idx_agent_tiers_active ON agent_tiers(is_active);

COMMENT ON TABLE agent_tiers IS 'Evidence-based tier definitions with performance expectations';

DO $$
BEGIN
    RAISE NOTICE '✓ Created agent_tiers table';
END $$;
...
DECLARE
    tier_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO tier_count FROM agent_tiers;
    RAISE NOTICE '✓ Seeded % tier definitions', tier_count;
END $$;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '--- Section 4: Creating Panel Configuration Tables ---';
END $$;
...
CREATE INDEX IF NOT EXISTS idx_panel_configurations_tenant ON panel_configurations(tenant_id);
CREATE INDEX IF NOT EXISTS idx_panel_configurations_type ON panel_configurations(panel_type);
CREATE INDEX IF NOT EXISTS idx_panel_configurations_active ON panel_configurations(is_active);
...
CREATE INDEX IF NOT EXISTS idx_panel_config_members_panel ON panel_configuration_members(panel_configuration_id);
CREATE INDEX IF NOT EXISTS idx_panel_config_members_agent ON panel_configuration_members(agent_id);

DO $$
BEGIN
    RAISE NOTICE '✓ Created panel_configurations and panel_configuration_members tables';
END $$;

-- ==========================================
-- SECTION 5: SAFETY TRIGGERS
-- ==========================================

DO $$
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '--- Section 5: Creating Safety Triggers Table ---';
END $$;
...
CREATE INDEX IF NOT EXISTS idx_safety_triggers_type ON safety_triggers(trigger_type);
CREATE INDEX IF NOT EXISTS idx_safety_triggers_active ON safety_triggers(is_active);
CREATE INDEX IF NOT EXISTS idx_safety_triggers_clinical ON safety_triggers(is_clinical_safety);
...
BEGIN
    SELECT COUNT(*) INTO trigger_count FROM safety_triggers;
    RAISE NOTICE '✓ Seeded % safety triggers', trigger_count;
END $$;

-- ==========================================
-- SECTION 6: KG TYPE REGISTRIES
-- ==========================================

DO $$
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '--- Section 6: Creating KG Type Registry Tables ---';
END $$;
...
CREATE INDEX IF NOT EXISTS idx_kg_node_types_domain ON kg_node_types(domain);
CREATE INDEX IF NOT EXISTS idx_kg_node_types_active ON kg_node_types(is_active);

COMMENT ON TABLE kg_node_types IS 'Registry of knowledge graph node types';
...
CREATE INDEX IF NOT EXISTS idx_kg_edge_types_category ON kg_edge_types(relationship_category);
CREATE INDEX IF NOT EXISTS idx_kg_edge_types_active ON kg_edge_types(is_active);

COMMENT ON TABLE kg_edge_types IS 'Registry of knowledge graph edge types';
...
BEGIN
    SELECT COUNT(*) INTO node_type_count FROM kg_node_types;
    SELECT COUNT(*) INTO edge_type_count FROM kg_edge_types;
    RAISE NOTICE '✓ Seeded % node types and % edge types', node_type_count, edge_type_count;
END $$;

-- ==========================================
-- SECTION 7: ADDITIONAL VIEWS
-- ==========================================

DO $$
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '--- Section 7: Creating Additional Views ---';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✓ Created 2 additional views';
END $$;
...
DECLARE
    table_count INTEGER;
    view_count INTEGER;
    total_seeded INTEGER;
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '=================================================================';
    RAISE NOTICE 'AGENTOS 3.0 ADDITIONAL SCHEMA - VERIFICATION';
    RAISE NOTICE '=================================================================';
    RAISE NOTICE '';
    ...
    WHERE table_schema = 'public'
      AND table_name IN ('v_agent_selection_ready', 'v_agent_performance_dashboard');
    
    -- Count seeded records
    SELECT 
        (SELECT COUNT(*) FROM agent_tiers) +
        (SELECT COUNT(*) FROM safety_triggers) +
        (SELECT COUNT(*) FROM kg_node_types) +
        (SELECT COUNT(*) FROM kg_edge_types)
    INTO total_seeded;
    
    RAISE NOTICE '=== SUMMARY ===';
    RAISE NOTICE 'New tables created: %', table_count;
    RAISE NOTICE 'New views created: %', view_count;
    RAISE NOTICE 'Records seeded: %', total_seeded;
    RAISE NOTICE '';
    RAISE NOTICE '=================================================================';
    RAISE NOTICE 'AGENTOS 3.0 FOUNDATION COMPLETE!';
    RAISE NOTICE '=================================================================';
    RAISE NOTICE '';
    RAISE NOTICE 'Total Schema:';
    RAISE NOTICE '  • AgentOS 2.0: 34 tables + 6 views';
    RAISE NOTICE '  • AgentOS 3.0: 8 tables + 2 views';
    RAISE NOTICE '  • TOTAL: 42 tables + 8 views';
    RAISE NOTICE '';
    RAISE NOTICE 'Ready for Phase 1 implementation (GraphRAG Service)!';
    RAISE NOTICE '=================================================================';
END $$;

-- Detailed verification queries
SELECT 'Agent Tiers' as entity, COUNT(*) as count FROM agent_tiers
UNION ALL SELECT 'Safety Triggers', COUNT(*) FROM safety_triggers
UNION ALL SELECT 'KG Node Types', COUNT(*) FROM kg_node_types
UNION ALL SELECT 'KG Edge Types', COUNT(*) FROM kg_edge_types;
