-- =============================================
-- Clean Up Soft-Deleted Field Medical Duplicates
-- Then Fix Active Role Mappings
-- =============================================
...
DECLARE
    ...
    updated_count INT := 0;
BEGIN
    RAISE NOTICE '=== Field Medical Duplicate Cleanup & Mapping Fix ===';
    RAISE NOTICE '';

    -- STEP 1: Permanently delete soft-deleted duplicates that are blocking updates
    RAISE NOTICE '--- Step 1: Remove soft-deleted duplicates ---';
    
    DELETE FROM public.org_roles
    WHERE slug = 'medical-field-trainer'
      AND deleted_at IS NOT NULL;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE '✓ Permanently deleted % soft-deleted "Field Medical Trainer" records', deleted_count;

    -- Check for other problematic soft-deleted Field Medical roles
    DELETE FROM public.org_roles
    WHERE slug IN (
    ...
    AND (
        ...
        OR tenant_id = '00000000-0000-0000-0000-000000000001' -- VITAL Platform
    );
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE '✓ Permanently deleted % other soft-deleted Field Medical role duplicates', deleted_count;

    RAISE NOTICE '';
    RAISE NOTICE '--- Step 2: Update active roles with correct mappings ---';

    -- Now proceed with updates (same as before)
    ...
    WHERE department_id = field_medical_dept_id
      ...
      AND deleted_at IS NULL;
    RAISE NOTICE '✓ % Field Medical roles correctly mapped', updated_count;
