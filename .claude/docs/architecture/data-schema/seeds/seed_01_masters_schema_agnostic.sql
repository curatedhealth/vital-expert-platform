...
-- Part 1: Master Agents (Level 1) - SCHEMA AGNOSTIC
-- =====================================================
-- Uses only confirmed existing columns from agents table
-- Uses metadata JSONB for 5-level hierarchy attributes
-- =====================================================
...
BEGIN
    -- Get tenant ID (assuming pharmaceuticals tenant exists)
    ...
    RAISE NOTICE 'Using schema-agnostic approach with metadata';
    RAISE NOTICE '========================================';
    ...
    INSERT INTO agents (
        ...
        jsonb_build_object(
            ...
            'reasoning_capabilities', jsonb_build_array('strategic_planning', 'delegation', 'coordination'),
            'domain_expertise', jsonb_build_array('medical_affairs_strategy', 'cross_functional_coordination')
        )
    ) RETURNING id INTO v_master_medical_affairs;
    
    RAISE NOTICE 'Created Master Agent 1: Medical Affairs Strategy Master (ID: %)', v_master_medical_affairs;
    ...
    INSERT INTO agents (
        ...
        jsonb_build_object(
            ...
            'industry_vertical', 'pharmaceuticals',
            'reasoning_capabilities', jsonb_build_array('protocol_optimization', 'data_quality_oversight', 'patient_safety'),
            'domain_expertise', jsonb_build_array('clinical_operations', 'trial_management', 'site_coordination')
        )
    ) RETURNING id INTO v_master_clinical_ops;
    
    RAISE NOTICE 'Created Master Agent 2: Clinical Operations Master (ID: %)', v_master_clinical_ops;
    ...
    INSERT INTO agents (
        ...
        jsonb_build_object(
            ...
            'max_spawned_agents', 10,
            'industry_vertical', 'pharmaceuticals',
            'reasoning_capabilities', jsonb_build_array('content_strategy', 'publication_planning', 'brand_consistency'),
            'domain_expertise', jsonb_build_array('scientific_writing', 'medical_communications', 'regulatory_documentation')
        )
    ) RETURNING id INTO v_master_scientific_comms;
    
    RAISE NOTICE 'Created Master Agent 3: Scientific Communications Master (ID: %)', v_master_scientific_comms;
    ...
    INSERT INTO agents (
        ...
        jsonb_build_object(
            ...
            'max_spawned_agents', 8,
            'industry_vertical', 'pharmaceuticals',
            'reasoning_capabilities', jsonb_build_array('evidence_synthesis', 'economic_modeling', 'value_demonstration'),
            'domain_expertise', jsonb_build_array('real_world_evidence', 'health_economics', 'outcomes_research', 'market_access')
        )
    ) RETURNING id INTO v_master_evidence_strategy;
    
    RAISE NOTICE 'Created Master Agent 4: Evidence & HEOR Strategy Master (ID: %)', v_master_evidence_strategy;
    ...
    INSERT INTO agents (
        ...
        jsonb_build_object(
            ...
            'can_spawn_specialists', true,
            'can_spawn_workers', true,
            'max_spawned_agents', 12,
            'industry_vertical', 'pharmaceuticals',
            'reasoning_capabilities', jsonb_build_array('kol_strategy', 'regional_coordination', 'scientific_exchange'),
            'domain_expertise', jsonb_build_array('field_medical', 'msl_operations', 'kol_engagement', 'hcp_education')
        )
    ) RETURNING id INTO v_master_field_operations;
    
    RAISE NOTICE 'Created Master Agent 5: Field Medical Operations Master (ID: %)', v_master_field_operations;
    
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Master Agents Summary:';
    RAISE NOTICE 'Total Created: 5';
    RAISE NOTICE 'Medical Affairs Strategy Master: %', v_master_medical_affairs;
    RAISE NOTICE 'Clinical Operations Master: %', v_master_clinical_ops;
    RAISE NOTICE 'Scientific Communications Master: %', v_master_scientific_comms;
    RAISE NOTICE 'Evidence & HEOR Strategy Master: %', v_master_evidence_strategy;
    RAISE NOTICE 'Field Medical Operations Master: %', v_master_field_operations;
    RAISE NOTICE '========================================';
...
SELECT 
    ...
    jsonb_agg(jsonb_build_object('name', name, 'slug', slug, 'level', metadata->'agent_level')) as agents
FROM agents
WHERE metadata->>'agent_level' = '1'
