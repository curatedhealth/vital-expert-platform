...
def get_jtbds_by_code_prefix(prefix):
    """Get JTBDs with proper URL encoding"""
    # Use %25 for literal % in URL
    encoded_prefix = urllib.parse.quote(prefix, safe='')
    url = f"{URL}/rest/v1/jtbd?select=id,code,name,functional_area&code=like.{encoded_prefix}%25&limit=50"
...
def main():
    ...
    for pat in patterns:
        jtbds = get_jtbds_by_code_prefix(pat)
