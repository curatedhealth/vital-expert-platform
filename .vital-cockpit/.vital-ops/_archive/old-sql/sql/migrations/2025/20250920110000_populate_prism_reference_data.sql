...
-- VERSION: Fixed with conflict handling for idempotent operations
...
-- Insert all prompt categories with conflict handling
-- Use a DO block to handle the category insertions with proper conflict resolution
DO $$
DECLARE
    ...
    commercial_id UUID;
    marketing_id UUID;
    patient_advocacy_id UUID;
    data_analytics_id UUID;
    operations_id UUID;
BEGIN
    -- Get domain IDs
    SELECT id INTO medical_affairs_id FROM prompt_domains WHERE name = 'medical_affairs';
    SELECT id INTO compliance_id FROM prompt_domains WHERE name = 'compliance';
    SELECT id INTO commercial_id FROM prompt_domains WHERE name = 'commercial';
    SELECT id INTO marketing_id FROM prompt_domains WHERE name = 'marketing';
    SELECT id INTO patient_advocacy_id FROM prompt_domains WHERE name = 'patient_advocacy';
    SELECT id INTO data_analytics_id FROM prompt_domains WHERE name = 'data_analytics';
    SELECT id INTO operations_id FROM prompt_domains WHERE name = 'operations';

    -- Medical Affairs Categories
    INSERT INTO prompt_categories (domain_id, name, display_name, description, sort_order) VALUES
    (medical_affairs_id, 'clinical_research', 'Clinical Research', 'Protocol development, study design, and research analysis', 1),
    (medical_affairs_id, 'medical_writing', 'Medical Writing', 'Scientific publications, regulatory documents, and clinical reports', 2),
    (medical_affairs_id, 'regulatory_affairs', 'Regulatory Affairs', 'Submission preparation, regulatory strategy, and compliance documentation', 3),
    (medical_affairs_id, 'scientific_communication', 'Scientific Communication', 'Conference presentations, scientific posters, and expert communications', 4),
    (medical_affairs_id, 'medical_information', 'Medical Information', 'Medical inquiries, safety reporting, and information management', 5)
    ...
    INSERT INTO prompt_categories (domain_id, name, display_name, description, sort_order) VALUES
    (compliance_id, 'regulatory_compliance', 'Regulatory Compliance', 'FDA, EMA, and other regulatory body compliance requirements', 1),
    (compliance_id, 'quality_assurance', 'Quality Assurance', 'QA processes, audits, and quality management systems', 2),
    (compliance_id, 'risk_management', 'Risk Management', 'Risk assessment, mitigation strategies, and compliance monitoring', 3),
    (compliance_id, 'policy_management', 'Policy Management', 'Policy development, implementation, and training', 4),
    (compliance_id, 'audit_preparation', 'Audit Preparation', 'Internal and external audit preparation and response', 5)
    ON CONFLICT (domain_id, name) DO UPDATE SET
        display_name = EXCLUDED.display_name,
        description = EXCLUDED.description,
        sort_order = EXCLUDED.sort_order;
    ...
    INSERT INTO prompt_categories (domain_id, name, display_name, description, sort_order) VALUES
    (commercial_id, 'sales_enablement', 'Sales Enablement', 'Sales training, tools, and performance optimization', 1),
    (commercial_id, 'market_access', 'Market Access', 'Payer engagement, value demonstration, and access strategies', 2),
    (commercial_id, 'business_development', 'Business Development', 'Partnership strategies, deal evaluation, and growth planning', 3),
    (commercial_id, 'competitive_intelligence', 'Competitive Intelligence', 'Market analysis, competitor monitoring, and strategic insights', 4),
    (commercial_id, 'launch_planning', 'Launch Planning', 'Product launch strategies, timeline management, and execution planning', 5)
    ON CONFLICT (domain_id, name) DO UPDATE SET
        display_name = EXCLUDED.display_name,
        description = EXCLUDED.description,
        sort_order = EXCLUDED.sort_order;
    ...
    INSERT INTO prompt_categories (domain_id, name, display_name, description, sort_order) VALUES
    (marketing_id, 'content_marketing', 'Content Marketing', 'Educational content, thought leadership, and digital marketing', 1),
    (marketing_id, 'brand_management', 'Brand Management', 'Brand strategy, positioning, and messaging consistency', 2),
    (marketing_id, 'digital_campaigns', 'Digital Campaigns', 'Online marketing, social media, and digital engagement strategies', 3),
    (marketing_id, 'stakeholder_engagement', 'Stakeholder Engagement', 'KOL engagement, advisory boards, and relationship management', 4),
    (marketing_id, 'market_research', 'Market Research', 'Customer insights, market analysis, and research methodologies', 5)
    ON CONFLICT (domain_id, name) DO UPDATE SET
        display_name = EXCLUDED.display_name,
        description = EXCLUDED.description,
        sort_order = EXCLUDED.sort_order;

    -- Patient Advocacy Categories
    INSERT INTO prompt_categories (domain_id, name, display_name, description, sort_order) VALUES
    (patient_advocacy_id, 'patient_engagement', 'Patient Engagement', 'Patient communication, education, and engagement strategies', 1),
    (patient_advocacy_id, 'support_programs', 'Support Programs', 'Patient assistance, adherence programs, and support services', 2),
    (patient_advocacy_id, 'advocacy_initiatives', 'Advocacy Initiatives', 'Patient advocacy campaigns, policy initiatives, and awareness programs', 3),
    (patient_advocacy_id, 'patient_education', 'Patient Education', 'Educational materials, resources, and communication strategies', 4),
    (patient_advocacy_id, 'outcomes_research', 'Outcomes Research', 'Patient-reported outcomes, real-world evidence, and impact assessment', 5)
    ON CONFLICT (domain_id, name) DO UPDATE SET
        display_name = EXCLUDED.display_name,
        description = EXCLUDED.description,
        sort_order = EXCLUDED.sort_order;
    ...
    INSERT INTO prompt_categories (domain_id, name, display_name, description, sort_order) VALUES
    (data_analytics_id, 'business_intelligence', 'Business Intelligence', 'KPI analysis, dashboard development, and performance metrics', 1),
    (data_analytics_id, 'predictive_analytics', 'Predictive Analytics', 'Forecasting, trend analysis, and predictive modeling', 2),
    (data_analytics_id, 'real_world_evidence', 'Real World Evidence', 'RWE studies, data analysis, and evidence generation', 3),
    (data_analytics_id, 'data_visualization', 'Data Visualization', 'Chart creation, infographic development, and visual storytelling', 4),
    (data_analytics_id, 'reporting_automation', 'Reporting Automation', 'Automated reporting, data processing, and workflow optimization', 5)
    ON CONFLICT (domain_id, name) DO UPDATE SET
        display_name = EXCLUDED.display_name,
        description = EXCLUDED.description,
        sort_order = EXCLUDED.sort_order;
    ...
    INSERT INTO prompt_categories (domain_id, name, display_name, description, sort_order) VALUES
    (operations_id, 'process_optimization', 'Process Optimization', 'Workflow improvement, efficiency analysis, and process redesign', 1),
    (operations_id, 'project_management', 'Project Management', 'Project planning, execution, and performance monitoring', 2),
    (operations_id, 'resource_management', 'Resource Management', 'Resource allocation, capacity planning, and optimization', 3),
    (operations_id, 'vendor_management', 'Vendor Management', 'Supplier evaluation, contract management, and vendor relationships', 4),
    (operations_id, 'change_management', 'Change Management', 'Organizational change, transformation planning, and adoption strategies', 5)
    ON CONFLICT (domain_id, name) DO UPDATE SET
        display_name = EXCLUDED.display_name,
        description = EXCLUDED.description,
        sort_order = EXCLUDED.sort_order;
...
-- Insert Prompt Templates (with conflict handling)
