-- ============================================================================
-- VITAL PLATFORM - MULTI-TENANT RESOURCES (FIXED - No deleted_at references)
-- Migration: Add Tenant Columns to Resource Tables
-- Version: 1.0.1 (FIXED)
-- Date: 2025-10-26
-- ============================================================================
-- Purpose: Add tenant ownership and sharing columns to resource tables
-- FIXED: Removed deleted_at references (column doesn't exist in remote schema)
-- ============================================================================
...
-- Create indexes for tenant-based queries (WITHOUT deleted_at filter)
...
-- ============================================================================
-- 2. CREATE TOOLS TABLE WITH TENANT SUPPORT
-- ============================================================================
...
-- Create indexes for tools (WITHOUT deleted_at)
...
CREATE INDEX IF NOT EXISTS idx_tools_resource_type ON tools(resource_type);
...
-- ============================================================================
-- 3. CREATE PROMPTS TABLE WITH TENANT SUPPORT
-- ============================================================================
...
-- Create indexes for prompts (WITHOUT deleted_at)
...
CREATE INDEX IF NOT EXISTS idx_prompts_resource_type ON prompts(resource_type);
...
-- ============================================================================
-- 4. CREATE WORKFLOWS TABLE WITH TENANT SUPPORT
-- ============================================================================
...
-- Create indexes for workflows (WITHOUT deleted_at)
CREATE INDEX IF NOT EXISTS idx_workflows_tenant ON workflows(tenant_id);
CREATE INDEX IF NOT EXISTS idx_workflows_sharing ON workflows(is_shared, sharing_mode) WHERE is_shared = true;
CREATE INDEX IF NOT EXISTS idx_workflows_resource_type ON workflows(resource_type);
...
BEGIN
    ...
    RAISE NOTICE '✅ Migration 20251026000002 (FIXED) completed successfully';
    RAISE NOTICE '   Added % tenant columns to agents table', v_column_count;
    RAISE NOTICE '   Created tables: tools, prompts, workflows';
