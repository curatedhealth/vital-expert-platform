#!/usr/bin/env python3
"""
MSL Personas Transformation Script - Phase 1 (Final - Schema-Correct)
Uses actual column names from Supabase schema
"""
import json
...
JSON_FILE = "/Users/hichamnaim/Downloads/MARKET_ACCESS_207_PERSONAS_3_5_PER_ROLE.json"
OUTPUT_SQL = "DEPLOY_MARKET_ACCESS_207.sql"
...
# Generate SQL
sql = f"""-- MSL Personas Phase 1 - Deployment (Final Corrected)
...
"""
    ...
    # Goals (goal_type, goal_text, priority)
...
"""
    ...
    # Pain Points (pain_point_text, pain_category, severity)
    for p_idx, pain in enumerate(persona.get('pain_points', []), 1):
        pain_text = pain.get('pain', 'Pain point')
        severity = pain.get('severity', 'medium')
        # Map severity values (valid: critical, high, medium, low)
        severity_map = {'very_high': 'critical', 'high': 'high', 'medium': 'medium', 'low': 'low'}
...
"""
    ...
    # Challenges (challenge_text, challenge_type)
    for c_idx, challenge in enumerate(persona.get('challenges', []), 1):
        challenge_text = challenge.get('challenge', 'Challenge')
...
"""
    ...
    # Responsibilities (responsibility_text, responsibility_type)
...
"""
    ...
    # Tools (tool_name, usage_frequency, proficiency_level)
    for t_idx, tool in enumerate(persona.get('tools', []), 1):
        tool_name = tool.get('tool', 'Tool')
        usage_freq = tool.get('usage', tool.get('usage_frequency', 'weekly'))
        # Map usage_frequency values (valid: daily, weekly, monthly, occasional, rarely)
        freq_map = {'daily': 'daily', 'weekly': 'weekly', 'monthly': 'monthly',
                    'quarterly': 'monthly', 'occasional': 'occasional', 'rarely': 'rarely',
                    'annually': 'rarely'}
        usage_freq = freq_map.get(usage_freq, 'weekly')

        proficiency = tool.get('proficiency', 'competent')
        # Map proficiency values
        prof_map = {'expert': 'expert', 'advanced': 'proficient', 'intermediate': 'competent', 'beginner': 'beginner'}
        proficiency = prof_map.get(proficiency, 'competent')
    ...
    INSERT INTO persona_tools (
        ...
        {escape_sql(proficiency)},
        {t_idx}
    );

"""

    # Internal Stakeholders (stakeholder_role, interaction_frequency, relationship_type, influence_level)
    for s_idx, stakeholder in enumerate(persona.get('internal_stakeholders', []), 1):
        stake_role = stakeholder.get('role', 'Stakeholder')
...
"""
    ...
    # External Stakeholders (different structure than internal)
    ...
    INSERT INTO persona_external_stakeholders (
        id, persona_id, tenant_id,
        stakeholder_name, stakeholder_type, relationship_importance
    ) VALUES (
        gen_random_uuid(), v_persona_id, '{TENANT_ID}'::uuid,
        {escape_sql(stake_role)},
        'partner',
...
print(f'   psql "postgresql://..." -f {OUTPUT_SQL}')
