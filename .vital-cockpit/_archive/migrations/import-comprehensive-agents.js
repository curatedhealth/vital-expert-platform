...
function processMedicalAffairsAgents(jsonData) {
  ...
  return jsonData.agents.map(agent => {
    // Map department names to match organizational structure
    const departmentMapping = {
      'Field Medical': 'Medical Science Liaisons',
      'Medical Information': 'Medical Information',
      'Medical Communications & Writing': 'Medical Communications',
      'Evidence Generation & HEOR': 'HEOR',
      'Clinical Operations Support': 'Clinical Operations',
      'Medical Excellence & Governance': 'Medical Affairs',
      'Medical Strategy & Operations': 'Medical Affairs'
    };
    
    const mappedDepartment = departmentMapping[agent.department] || 'Medical Affairs';
...
function processMarketAccessAgents(jsonData) {
  ...
  return jsonData.agents.map(agent => {
    ...
    const departmentMapping = {
      ...
      'Payer Strategy & Contracting': 'Market Access',
      'Pricing & Reimbursement': 'Market Access',
      'Patient Access & Hub Services': 'Market Access',
      'Policy & Government Affairs': 'Market Access',
      'Market Access Marketing & Communications': 'Marketing',
      'Market Access Operations & Analytics': 'Market Access'
    };
    
    const mappedDepartment = departmentMapping[agent.department] || 'Market Access';
...
function processMarketingAgents(jsonData) {
  ...
  return jsonData.agents.map(agent => {
    ...
    const mappedDepartment = departmentMapping[agent.department] || 'Marketing';
...
async function importComprehensiveAgents() {
  try {
    console.log('🚀 IMPORTING COMPREHENSIVE AGENT COLLECTIONS');
    console.log('======================================================================\n');
    ...
    const { data: uploadedAgents, error: verifyError } = await supabase
      .from('agents')
      .select('id, name, display_name, business_function, department, status')
    ...
    sampleAgents.forEach((agent, index) => {
      console.log(`${index + 1}. ${agent.display_name} (${agent.business_function} - ${agent.department})`);
    });
