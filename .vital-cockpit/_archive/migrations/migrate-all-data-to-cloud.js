...
console.log('🚀 Starting comprehensive VITAL Expert data migration to Supabase Cloud...');
...
async function clearTable(tableName) {
  ...
  try {
    await makeRequest(`${SUPABASE_URL}/rest/v1/${tableName}?id=not.is.null`, {
      method: 'DELETE'
    });
...
async function processAgentsFromCSV() {
  ...
  // Track used display names to avoid duplicates
  const usedDisplayNames = new Set();
  
  return agents.map((agent, index) => {
    // Map CSV columns to database schema
    let displayName = agent.Display_Name || agent.Name || `Agent ${index + 1}`;
    
    // Make display name unique if it already exists
    let counter = 1;
    ...
    return {
      ...
      color: agent.Avatar ? '#3B82F6' : '#10B981',
      ...
      cost_per_query: Math.min(parseFloat(agent.Cost_Per_Execution) || 0.05, 9.9999),
      validation_status: agent.Validation_Status || 'pending',
      performance_metrics: {
        "accuracy": parseFloat(agent.Accuracy_Score) || 0.85,
        "response_time": parseInt(agent.Response_Time) || 2000
      },
      ...
      medical_accuracy_score: Math.min(parseFloat(agent.Accuracy_Score) || 0.85, 1.0),
      citation_accuracy: 0.90,
      hallucination_rate: 0.05,
      medical_error_rate: 0.02,
      average_latency_ms: parseInt(agent.Response_Time) || 2000,
...
async function migrateAllData() {
  try {
    ...
    await clearTable('knowledge_domains');
    await clearTable('llm_providers');
    console.log('✅ Existing data cleared\n');

    // Step 2: Process and upload LLM providers
    console.log('📋 Step 2: Processing LLM providers...');
    ...
    // Step 3: Process and upload knowledge domains
    console.log('📋 Step 3: Processing knowledge domains...');
    ...
    // Step 4: Process and upload agents
    console.log('📋 Step 4: Processing agents...');
