-- =====================================================================================
-- UC_RA_006: FDA Breakthrough Designation Strategy - Part 1: Workflows & Tasks
-- =====================================================================================
...
SELECT sc.tenant_id, 'UC_RA_006', 'USC-RA-006', 'FDA Breakthrough Designation Strategy', 'RA',
  'Strategic assessment and application for FDA Breakthrough Device Designation for expedited review',
  'EXPERT', 120,
  json_build_array('Device description', 'Life-threatening/debilitating indication', 'Clinical data or preliminary evidence', 'Unmet medical need assessment')::jsonb,
  json_build_array('Breakthrough Designation application', 'Eligibility assessment', 'Unmet need justification', 'Preliminary data summary', 'Expedited pathway strategy')::jsonb,
  jsonb_build_object('designation_success', '70% application grant rate', 'time_to_market', '25% reduction in approval timeline')
...
WHERE uc.code = 'UC_RA_006' AND uc.tenant_id = sc.tenant_id
...
SELECT 'UC_RA_006 Part 1 Seeded' as status, COUNT(*) as tasks FROM dh_task t
INNER JOIN dh_workflow wf ON wf.id = t.workflow_id INNER JOIN dh_use_case uc ON uc.id = wf.use_case_id
WHERE uc.code = 'UC_RA_006' AND uc.tenant_id = (SELECT tenant_id FROM session_config);
