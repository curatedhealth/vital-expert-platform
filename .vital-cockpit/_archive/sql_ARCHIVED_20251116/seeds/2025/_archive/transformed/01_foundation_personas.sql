...
BEGIN
  -- Tenant ID directly assigned
  
  IF v_tenant_id IS NULL THEN
    RAISE EXCEPTION 'Tenant with ID 11111111-1111-1111-1111-111111111111 not found. Please create tenant first.';
  END IF;
...
CROSS JOIN (
  ...
  slug, name, expertise_level, years_experience,
  education, typical_titles, decision_authority, specializations,
...
DO UPDATE SET
  ...
  capabilities = EXCLUDED.specializations,
...
INSERT INTO personas (
  tenant_id, slug, name, expertise_level, years_experience,
  education, typical_titles, decision_authority, specializations,
  key_responsibilities, department, typical_availability_hours,
  response_time_sla_hours, description, metadata
)
SELECT 
  sc.tenant_id, p_data.slug, p_data.name, p_data., p_data.expertise_level, p_data.years_experience,
  p_data.education, p_data.typical_titles, p_data.decision_authority, p_data.specializations,
...
ON CONFLICT (tenant_id, code)
DO UPDATE SET
  name = EXCLUDED.name, unique_id = EXCLUDED.,
  ...
  decision_authority = EXCLUDED.decision_authority, capabilities = EXCLUDED.specializations,
