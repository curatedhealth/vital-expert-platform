...
description: Use this agent when you need to execute SQL operations, perform data migrations, transform JSON to SQL, introspect database schemas, validate data integrity, or deploy database changes to Supabase. This agent works as a specialist under data-architecture-expert for hands-on SQL operations. Examples:\n\n<example>\nContext: Data architecture expert needs to deploy schema changes.\ndata-architecture-expert: "I have a new schema design. I need you to generate the migration SQL and deploy it to Supabase."\nsql-supabase-specialist: "I'll introspect the current schema, compare it to your design, generate migration SQL with proper transactions, and execute the deployment."\n<uses Read, Bash, and SQL tools to complete migration>\n</example>\n\n<example>\nContext: Need to transform persona JSON data to SQL INSERTs.\ndata-architecture-expert: "We have 31 personas in JSON format that need to be inserted into the database. Can you handle the transformation?"\nsql-supabase-specialist: "I'll read the JSON, introspect the personas table schema to match column names exactly, generate SQL INSERTs with proper escaping, and create a deployment script."\n<generates SQL transformation script>\n</example>\n\n<example>\nContext: Schema mismatch error during deployment.\nuser: "Error: column 'employment_status' does not exist"\nsql-supabase-specialist: "I'll introspect the actual deployed schema, compare it to what the SQL expects, identify all mismatches, and regenerate the SQL to match the actual schema."\n<introspects schema and fixes SQL>\n</example>\n\n<example>\nContext: Need to verify data integrity after migration.\ndata-architecture-expert: "The migration completed. Can you verify all data was inserted correctly?"\nsql-supabase-specialist: "I'll run comprehensive validation queries: row counts, foreign key integrity checks, data type validations, and cross-reference with source data."\n<executes validation queries>\n</example>\n\n<example>\nContext: Performance issue with query.\ndata-architecture-expert: "This query is taking 5 seconds. Can you optimize it?"\nsql-supabase-specialist: "I'll run EXPLAIN ANALYZE, identify bottlenecks, check for missing indexes, and propose optimizations with before/after performance metrics."\n<analyzes and optimizes query>\n</example>
model: sonnet
color: blue
---
