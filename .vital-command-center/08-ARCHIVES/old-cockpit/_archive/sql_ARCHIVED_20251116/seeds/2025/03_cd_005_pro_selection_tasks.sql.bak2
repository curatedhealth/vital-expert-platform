...
-- =====================================================================================
-- SECTION 1A: AI AGENTS FOR UC_CD_005
-- =====================================================================================
-- AI Agents execute tasks autonomously using LangGraph/LLM
...
CROSS JOIN (
  VALUES
    ...
    (
      ...
      jsonb_build_object(
        ...
        'quality_thresholds', jsonb_build_object(
          ...
          'icc_min', 0.70,
          'responsiveness_effect_size_min', 0.3
        )
      )
    ),
    
    -- AGT-REGULATORY: Regulatory Compliance Agent
    (
      'AGT-REGULATORY',
      ...
      json_build_array(
        'FDA PRO Guidance compliance review',
        'COA Qualification assessment',
      ...
      jsonb_build_object(
        'agent_category', 'REGULATORY_REVIEW',
        'primary_guidance', json_build_array(
          'FDA PRO Guidance 2009',
          'FDA COA Qualification Program',
          'EMA Reflection Paper on PROs'
        ),
        'knowledge_domains', json_build_array('regulatory-affairs', 'fda-guidance', 'pro-qualification'),
        'citation_required', true
      )
    ),
    
    -- AGT-DIGITAL-FEASIBILITY: Digital Implementation Feasibility Agent
    (
      'AGT-DIGITAL-FEASIBILITY',
      'Digital Implementation Feasibility Agent',
      'AGT-DIGITAL-FEASIBILITY',
      'SPECIALIST',
      'AI agent specialized in assessing digital implementation feasibility for ePRO instruments',
      json_build_array(
        'ePRO platform compatibility assessment',
        'Digital migration evaluation',
        'UX/UI complexity analysis',
      ...
      jsonb_build_object(
        'agent_category', 'TECHNICAL_FEASIBILITY',
        'platforms', json_build_array('Medidata', 'Oracle', 'Veeva', 'signant', 'CRF Health'),
        'knowledge_domains', json_build_array('epro-platforms', 'digital-migration', 'ux-design'),
        'considerations', json_build_array(
          'Item complexity (text vs Likert vs visual)',
          'Recall period compatibility',
    ...
    -- AGT-PATIENT-BURDEN: Patient Burden Evaluation Agent
    (
      'AGT-PATIENT-BURDEN',
      'Patient Burden Evaluation Agent',
      'AGT-PATIENT-BURDEN',
      'SPECIALIST',
      'AI agent specialized in evaluating patient burden and acceptability of PRO instruments',
      json_build_array(
        'Instrument length assessment',
        'Completion time analysis',
        'Cognitive complexity evaluation',
        'Patient acceptability synthesis'
      )::jsonb,
      'SUPERVISED',
      jsonb_build_object(
        'model', 'claude-3-opus',
      ...
      jsonb_build_object(
        'agent_category', 'PATIENT_CENTRICITY',
        'evaluation_criteria', json_build_array(
          'Number of items',
          'Completion time',
          'Reading level',
          'Frequency of administration',
          'Patient-reported acceptability data'
        ),
        'knowledge_domains', json_build_array('patient-centricity', 'health-literacy', 'pro-burden'),
        'target_completion_time_minutes', 10,
        'target_reading_level', '6th-8th grade'
      )
    ),
    ...
    (
      ...
      jsonb_build_object(
        ...
        'synthesis_dimensions', json_build_array(
        ...
        'knowledge_domains', json_build_array('clinical-development', 'regulatory-strategy', 'pro-selection'),
        'output_format', 'Structured recommendation with tiered evidence'
      )
    ),
    ...
    (
      ...
      jsonb_build_object(
        'agent_category', 'COMMERCIAL_STRATEGY',
        'licensing_models', json_build_array(
          'Per-patient perpetual',
          'Annual subscription',
          'One-time study fee',
          'Public domain (no fee)'
        ),
        'knowledge_domains', json_build_array('pro-licensing', 'intellectual-property', 'commercial-strategy'),
        'considerations', json_build_array(
  ...
  code, name, unique_id, agent_type, description, 
  capabilities, autonomy_level, model_config, metadata
)
...
-- =====================================================================================
-- SECTION 1B: HUMAN PERSONAS FOR UC_CD_005
-- =====================================================================================
-- Human personas provide oversight, review, and approval for AI agent outputs
...
CROSS JOIN (
  VALUES
    ...
    (
      ...
      json_build_array('CMO', 'VP Medical Affairs', 'Chief Clinical Officer')::jsonb,
      'VERY_HIGH',
      json_build_array(
      ...
      'Senior executive providing strategic oversight and final approval for clinical endpoint and PRO selection',
      jsonb_build_object(
        'review_stage', 'FINAL_APPROVAL',
        'decision_scope', 'STRATEGIC',
        'typical_availability_hours', 10,
        'response_time_sla_hours', 72
      )
    ),
    ...
    (
      ...
      'EXPERT',
      '12-15',
      json_build_array('MD', 'MPH')::jsonb,
      json_build_array('VP Clinical Development', 'Head of Clinical Operations', 'Senior Clinical Director')::jsonb,
      'HIGH',
      json_build_array(
      ...
      json_build_array(
        'Clinical trial strategy approval',
        'Protocol oversight',
        'Endpoint validation',
      ...
      jsonb_build_object(
        ...
        'typical_availability_hours', 20,
        'response_time_sla_hours', 48
      )
    ),
    
    -- P03_CLINSCI: Senior Clinical Scientist
    (
      'P03_CLINSCI',
      ...
      'EXPERT',
      '10-15',
      json_build_array('PhD', 'PharmD', 'MD')::jsonb,
      json_build_array('Senior Clinical Scientist', 'Clinical Science Director', 'Principal Clinical Scientist')::jsonb,
      'HIGH',
      json_build_array(
      ...
      'Clinical Development',
      'Expert responsible for developing and validating clinical constructs and endpoints',
      jsonb_build_object(
    ...
    (
      ...
      'EXPERT',
      '10-15',
      json_build_array('PhD', 'MS Statistics')::jsonb,
      json_build_array('Principal Biostatistician', 'Director Biostatistics', 'Lead Statistician')::jsonb,
      'HIGH',
      json_build_array(
      ...
      'Biostatistics',
      'Expert statistician responsible for evaluating psychometric properties and statistical validity of PRO instruments',
      jsonb_build_object(
        'review_stage', 'TECHNICAL_REVIEW',
        'decision_scope', 'TECHNICAL',
    ...
    (
      ...
      json_build_array(
        ...
        'Regulatory risk evaluation',
        'PRO qualification strategy',
        'Agency interaction preparation'
      )::jsonb,
      'Regulatory Affairs',
      'Regulatory expert ensuring PRO selection aligns with FDA expectations and submission requirements',
      jsonb_build_object(
    ...
    -- P06_CLINOPS: Clinical Operations Director
    (
      'P06_CLINOPS',
      ...
      'ADVANCED',
      '8-12',
      json_build_array('RN', 'MBA', 'MS')::jsonb,
      json_build_array('Clinical Operations Director', 'Head of Site Management', 'VP Clinical Operations')::jsonb,
      'MEDIUM',
      json_build_array(
        'ePRO Implementation',
        'Site Feasibility',
      ...
      json_build_array(
        'ePRO platform selection and implementation',
        'Site training and support',
        'Operational feasibility assessment',
        'Patient burden optimization'
      )::jsonb,
      'Clinical Operations',
      'Operations leader ensuring practical implementation feasibility of PRO instruments in clinical trials',
      jsonb_build_object(
    ...
    -- P07_MEDWRITER: Medical Writer
    (
      'P07_MEDWRITER',
      'Medical Writer',
      'PERSONA-P07-MEDWRITER',
      'ADVANCED',
      '5-10',
      json_build_array('PhD', 'PharmD', 'MS', 'CMPP')::jsonb,
      json_build_array('Medical Writer', 'Senior Medical Writer', 'Scientific Writer')::jsonb,
      'LOW',
      json_build_array(
      ...
      json_build_array(
        'Endpoint justification documentation',
        'PRO selection rationale writing',
        'Regulatory submission document drafting',
        'Scientific literature synthesis'
      )::jsonb,
      'Clinical Development',
      'Professional writer responsible for documenting and communicating PRO selection rationale and justification',
      jsonb_build_object(
        'review_stage', 'DOCUMENTATION',
        'decision_scope', 'ADVISORY',
        'typical_availability_hours', 40,
        'response_time_sla_hours', 72
      )
    )
) AS p_data(
...
INSERT INTO dh_tool (tenant_id, code, name, category, vendor, unique_id, notes, metadata)
SELECT 
  sc.tenant_id,
  t_data.code,
  t_data.name,
...
) AS t_data(code, name, category, vendor, unique_id, notes, metadata)
ON CONFLICT (tenant_id, code)
DO UPDATE SET
  name = EXCLUDED.name,
...
INSERT INTO dh_rag_source (tenant_id, code, name, source_type, uri, description, unique_id, metadata)
SELECT 
  sc.tenant_id,
  rag_data.code,
  rag_data.name,
...
) AS rag_data(code, name, source_type, uri, description, unique_id, metadata)
ON CONFLICT (tenant_id, code)
DO UPDATE SET
  name = EXCLUDED.name,
  ...
  VALUES
    ...
    (
      ...
      jsonb_build_object(
        ...
        'decision_criteria', jsonb_build_object(
...
) AS task_data(code, title, objective, position, unique_id, extra)
...
CROSS JOIN (
  VALUES
    ...
    ('TSK-CD-005-001-02', 'TSK-CD-005-001-01', 'Sequential dependency: Domain identification requires construct definition'),
    -- Task 3 depends on Task 2
    ('TSK-CD-005-001-03', 'TSK-CD-005-001-02', 'Sequential dependency: COA type selection requires domain breakdown'),
    -- Task 4 depends on Task 1 (can work in parallel with Task 2-3)
    ('TSK-CD-005-001-04', 'TSK-CD-005-001-01', 'Sequential dependency: Temporal characteristics based on construct'),
    -- Task 5 depends on Task 1
    ('TSK-CD-005-001-05', 'TSK-CD-005-001-01', 'Sequential dependency: Contextual factors related to construct'),
    -- Task 6 depends on all previous tasks
    ('TSK-CD-005-001-06', 'TSK-CD-005-001-01', 'Sequential dependency: Documentation requires construct definition'),
    ('TSK-CD-005-001-06', 'TSK-CD-005-001-02', 'Sequential dependency: Documentation requires domain identification'),
    ('TSK-CD-005-001-06', 'TSK-CD-005-001-03', 'Sequential dependency: Documentation requires COA type selection'),
    ('TSK-CD-005-001-06', 'TSK-CD-005-001-04', 'Sequential dependency: Documentation requires temporal characteristics'),
    ('TSK-CD-005-001-06', 'TSK-CD-005-001-05', 'Sequential dependency: Documentation requires contextual factors')
) AS dep_data(child_unique_id, parent_unique_id, note)
...
CROSS JOIN (
  VALUES
    ...
    (
      ...
      'Primary agent for defining clinical construct',
      jsonb_build_object(
        'required_capabilities', json_build_array(
          'Clinical construct identification',
          'Domain decomposition',
          'Clinical meaningfulness assessment'
        ),
        'required_knowledge', json_build_array('clinical-development', 'pro-methodology', 'endpoint-selection'),
  ...
  task_unique_id, agent_code, assignment_type, execution_order,
  requires_human_approval, approval_persona_code, config_overrides,
  notes, metadata
)
JOIN dh_task t ON t.tenant_id = sc.tenant_id AND t.unique_id = ta_data.task_unique_id
JOIN dh_agent agent ON agent.tenant_id = sc.tenant_id AND agent.code = ta_data.agent_code
...
-- =====================================================================================
-- SECTION 7: TASK-PERSONA ASSIGNMENTS (Human Supervisors/Reviewers)
-- =====================================================================================

INSERT INTO dh_task_persona (tenant_id, task_id, persona_id, responsibility, review_timing)
SELECT 
  sc.tenant_id,
  t.id as task_id,
  p.id as persona_id,
...
CROSS JOIN (
  VALUES
    -- Task 1: Define Primary Clinical Construct
    ('TSK-CD-005-001-01', 'P01_CMO', 'APPROVE', 'AFTER_AGENT_RUNS'),
    ('TSK-CD-005-001-01', 'P02_VPCLIN', 'REVIEW', 'AFTER_AGENT_RUNS'),
    
    -- Task 2: Identify Construct Domains
    ('TSK-CD-005-001-02', 'P01_CMO', 'APPROVE', 'AFTER_AGENT_RUNS'),
    ('TSK-CD-005-001-02', 'P02_VPCLIN', 'REVIEW', 'AFTER_AGENT_RUNS'),
    
    -- Task 3: Determine PRO vs Other COA Types
    ('TSK-CD-005-001-03', 'P05_REGAFF', 'APPROVE', 'AFTER_AGENT_RUNS'),
    ('TSK-CD-005-001-03', 'P01_CMO', 'REVIEW', 'AFTER_AGENT_RUNS'),
    
    -- Task 4: Define Temporal Characteristics
    ('TSK-CD-005-001-04', 'P02_VPCLIN', 'APPROVE', 'AFTER_AGENT_RUNS'),
    ('TSK-CD-005-001-04', 'P04_BIOSTAT', 'REVIEW', 'AFTER_AGENT_RUNS'),
    
    -- Task 5: Specify Contextual Factors
    ('TSK-CD-005-001-05', 'P01_CMO', 'APPROVE', 'AFTER_AGENT_RUNS'),
    
    -- Task 6: Document Measurement Requirements
    ('TSK-CD-005-001-06', 'P02_VPCLIN', 'APPROVE', 'AFTER_AGENT_RUNS'),
    ('TSK-CD-005-001-06', 'P01_CMO', 'REVIEW', 'AFTER_AGENT_RUNS')
) AS tp_data(task_unique_id, persona_code, responsibility, review_timing)
JOIN dh_task t ON t.tenant_id = sc.tenant_id AND t.unique_id = tp_data.task_unique_id
JOIN dh_persona p ON p.tenant_id = sc.tenant_id AND p.code = tp_data.persona_code
...
-- List AI agents by type
SELECT 
  'AI Agents by Type' as status,
  agent_type as type,
  COUNT(*) as count
FROM dh_agent
WHERE tenant_id = (SELECT tenant_id FROM session_config)
...
FROM dh_task_agent_assignment taa
JOIN dh_task t ON taa.task_id = t.id
JOIN dh_agent a ON taa.agent_id = a.id
WHERE t.tenant_id = (SELECT tenant_id FROM session_config)
  AND t.unique_id LIKE 'TSK-CD-005-%'
...
SELECT 
  ...
  jsonb_build_object(
    'ai_agents', (SELECT COUNT(*) FROM dh_agent WHERE tenant_id = (SELECT tenant_id FROM session_config) AND code LIKE 'AGT-%'),
    'human_personas', (SELECT COUNT(*) FROM dh_persona WHERE tenant_id = (SELECT tenant_id FROM session_config) AND code LIKE 'P%'),
    ...
    'persona_assignments', (SELECT COUNT(*) FROM dh_task_persona tp JOIN dh_task t ON tp.task_id = t.id WHERE t.unique_id LIKE 'TSK-CD-005-%'),
