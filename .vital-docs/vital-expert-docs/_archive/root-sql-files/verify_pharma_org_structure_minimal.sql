-- =====================================================================
-- VERIFY PHARMACEUTICALS TENANT ORG-STRUCTURE DATA (MINIMAL VERSION)
-- Uses only essential columns that should definitely exist
-- =====================================================================
...
SELECT '=== ACTUAL TABLE STRUCTURE ===' as section;
...
SELECT 
    table_name,
    CASE WHEN COUNT(*) > 0 THEN 'EXISTS' ELSE 'MISSING' END as tenant_id_status
FROM information_schema.columns
WHERE table_schema = 'public'
    AND table_name IN ('org_functions', 'org_departments', 'org_roles')
...
-- =====================================================================
-- 3. COUNT DATA BY TENANT (USING ONLY ID AND TENANT_ID)
-- =====================================================================
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE function_id IS NULL) as departments_without_function
FROM org_departments
WHERE tenant_id = 'c6d221f8-1e8d-4dd9-86c5-d640ad6bf30b';
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE function_id IS NULL) as roles_without_function,
    COUNT(*) FILTER (WHERE department_id IS NULL) as roles_without_department
FROM org_roles
WHERE tenant_id = 'c6d221f8-1e8d-4dd9-86c5-d640ad6bf30b';
...
-- Roles pointing to functions from different tenants
SELECT 
...
-- Roles pointing to departments from different tenants
SELECT 
...
-- Functions with missing tenant_id
SELECT 
    'Functions with Missing tenant_id' as issue_type,
    COUNT(*) as issue_count
FROM org_functions
