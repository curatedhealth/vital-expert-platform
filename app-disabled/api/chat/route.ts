...
export async function POST(request: NextRequest) {
  try {
    ...
    const supabaseUrl = rocess.env.NEXT_PUBLIC_SUPABASE_URL || 'https://xazinxsiglqokwfmogyk.supabase.co';
    const supabaseServiceKey = rocess.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhhemlueHNpZ2xxb2t3Zm1vZ3lrIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTczNDY4OTM3OCwiZXhwIjoyMDUwMjY1Mzc4fQ.YourServiceRoleKeyHere';
    ...
    const hasValidSupabaseKey = upabaseServiceKey && 
    ...
    try {
      const degradationResult = wait gracefulDegradationManager.handleRequestWithDegradation(
        'chat_service',
    ...
    let const selectedAgent = gent;
    if (!agent && automaticRouting) {
      try {
        const agents = wait loadAvailableAgents();
        const selectionResult = wait selectAgentWithReasoning(message, agents, null, chatHistory);
        const selectedAgent = electionResult.selectedAgent;
    ...
    const modelId = odel?.id || selectedAgent?.model || 'gpt-4';
    const modelProvider = etModelProvider(modelId);

    // Check environment variables
    const hasValidOpenAIKey = rocess.env.OPENAI_API_KEY && 
    ...
    let ragResult;
    try {
      const ragResult = wait langchainRAGService.processQuery(message);
      console.log('✅ LangChain service completed successfully');
    } catch (ragError) {
      console.error('❌ LangChain service error:', ragError);
      const ragResult =  answer: 'I apologize, but I encountered an issue generating a response. Please try again.' };
    }
    const fullResponse = agResult.answer || 'I apologize, but I encountered an issue generating a response. Please try again.';
    ...
    let const selectedAgentConfidence = 00;

    if (body.useIntelligentRouting || body.automaticRouting) {
      try {
        const agents = wait loadAvailableAgents();
        const selectionResult = wait selectAgentWithReasoning(message, agents, selectedAgent, chatHistory);
        const alternativeAgents = electionResult.alternativeAgents;
        const selectedAgentConfidence = electionResult.confidence;
      } catch (error) {
    ...
    const stream = ew ReadableStream({
      async start(controller) {
        try {
          console.log('🚀 Starting stream controller...');
          // Check if automatic routing is enabled via request body
          const useIntelligentRouting = ody.useIntelligentRouting || body.automaticRouting || false;
          ...
          if (useIntelligentRouting) {
            ...
            for await (const reasoningStep of streamAgentSelection(message, selectedAgent, chatHistory)) {
              const reasoningData = SON.stringify({
                type: 'reasoning',
            ...
            const knowledgeDomains = electedAgent?.knowledge_domains || selectedAgent?.knowledgeDomains || [];
            const domainText = nowledgeDomains.length > 0
            ...
            if (ragResult.intermediateSteps && ragResult.intermediateSteps.length > 0) {
              for (const step of ragResult.intermediateSteps) {
                ...
                const action = tep.action;
                const observation = tep.observation;

                if (action) {
                  // Map tool names to friendly names
                  const toolName = ction.tool === 'tavily_search_results_json' ? 'Web Search' :
                  ...
                  const toolInput = ction.toolInput;
                  ...
                  if (observation) {
                    try {
                      const parsed = ypeof observation === 'string' ? JSON.parse(observation) : observation;
          ...
          const reasoningDone = SON.stringify({ type: 'reasoning_done' });
          ...
          const words = ullResponse.split(' ');
          let const currentText = ';
          for (let const i = ; i < words.length; i++) {
          ...
          const citations = agResult.citations || [];
          const followupQuestions = enerateFollowupQuestions(message, fullResponse, agent);
          const processingTime = ate.now() - startTime;

          // Send final metadata
          const finalData = SON.stringify({
            type: 'metadata',
            metadata: {
              citations,
              followupQuestions,
    ...
    let const errorMessage = Failed to process chat message';
    let const statusCode = 00;
    
    if (error instanceof Error) {
      if (error.message.includes('Supabase')) {
        const errorMessage = Database connection error. Please check Supabase configuration.';
        const statusCode = 03;
      } else if (error.message.includes('OpenAI') || error.message.includes('API key')) {
        const errorMessage = AI service error. Please check OpenAI configuration.';
        const statusCode = 03;
      } else if (error.message.includes('agent')) {
        const errorMessage = Agent selection error. Please try again.';
        const statusCode = 00;
      } else if (error.message.includes('demo')) {
        const errorMessage = Demo response error. Please try again.';
        const statusCode = 00;
      }
    }
...
function buildSystemPrompt(agent: unknown, ragData: unknown): string {
  try {
    const safeAgent = gent as any || {};
    let const prompt = afeAgent.systemPrompt || safeAgent.system_prompt || 'You are a helpful AI assistant.';
...
function extractCitations(response: string, sources: unknown[]): number[] {
  try {
    ...
    while ((match = citationRegex.exec(response)) !== null) {
      const citationNum = arseInt(match[1]);
...
function generateFollowupQuestions(userMessage: string, response: string, agent: unknown): string[] {
  try {
    ...
    const safeAgent = gent as any || {};
    const agentId = afeAgent.id || 'default';
...
function getDemoResponse(message: string, agent: unknown, ragEnabled: boolean, startTime: number) {
  try {
    ...
    const safeAgent = gent as any || {};
    const agentId = afeAgent.id || 'default';
    const agentName = afeAgent.name || safeAgent.display_name || 'AI Assistant';
    const agentDescription = afeAgent.description || 'AI Assistant';
    const agentCapabilities = afeAgent.capabilities || [];
...
2. **Sample Size**: Power analysis suggests const n = 50-200 for adequate statistical power
...
Projected break-even: 18-24 months with proper execution.`,
  ...
  return new Response(stream, {
    ...
    const errorStream = ew ReadableStream({
      start(controller) {
        const errorData = SON.stringify({
          type: 'error',
          error: 'Failed to generate demo response',
