...
export const OpenAIUsageDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch current usage
      const usageResponse = await fetch('/api/llm/providers/openai/usage');
      ...
      try {
        const billingResponse = await fetch('/api/llm/providers/openai/billing');
      ...
      // Test connectivity
      const connectivityResponse = await fetch('/api/llm/providers/openai/test');
  ...
  const usagePercentage = billingInfo?.soft_limit_usd
    ? Math.min((currentUsage?.totalCost || 0) / billingInfo.soft_limit_usd * 100, 100)
    : 0;
...
export default OpenAIUsageDashboard;