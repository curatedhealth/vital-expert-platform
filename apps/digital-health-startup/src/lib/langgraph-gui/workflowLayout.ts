...
function identifyExpertNodes(nodes: Node[]): Set<string> {
  ...
  nodes.forEach(node => {
    ...
    const taskName = node.data?.task?.name || node.data?.label || '';
...
function findExpertLevel(
  ...
  const moderatorNode = nodes.find(n => 
    n.data?.task?.id === 'moderator' ||
    (n.data?.task?.name || n.data?.label || '').toLowerCase().includes('moderator')
  );
  ...
  const phaseNode = nodes.find(n => {
    ...
    const taskName = (n.data?.task?.name || n.data?.label || '').toLowerCase();
...
export function getWorkflowPhases(nodes: Node[], edges: Edge[]): Array<{
  ...
  groups.forEach((levelNodes, level) => {
    ...
    if (levelNodes.length > 0) {
      ...
      const nodeName = firstNode.data?.task?.name || firstNode.data?.label || '';
