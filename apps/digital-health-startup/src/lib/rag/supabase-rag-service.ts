...
export class SupabaseRAGService {
  ...
  async updateDomainStats(domainName: string): Promise<void> {
    ...
    const totalSizeMB = (chunkStats?.reduce((sum, chunk) => sum + chunk.content_length, 0) || 0) / (1024 * 1024);
  ...
  private calculateContextRelevance(content: string, context: string[]): number {
    ...
    const contextWords = new Set(context.join(' ').toLowerCase().split(' '));

    const overlap = Array.from(contentWords).filter(word => contextWords.has(word)).length;
