...
export async function POST(request: NextRequest) {
  try {
    console.log('🗄️ [Database Setup] Creating missing tables');

    // Create user_agents table
    console.log('🔧 [Database Setup] Creating user_agents table');
    const userAgentsSQL = `
    ...
    const { error: userAgentsError } = await supabase.rpc('exec_sql', { sql: userAgentsSQL });
    if (userAgentsError) {
      console.warn('⚠️ [Database Setup] user_agents table warning:', userAgentsError.message);
    } else {
      console.log('✅ [Database Setup] user_agents table created');
    }

    // Create chat_sessions table
    console.log('🔧 [Database Setup] Creating chat_sessions table');
    ...
    const { error: chatSessionsError } = await supabase.rpc('exec_sql', { sql: chatSessionsSQL });
    if (chatSessionsError) {
      console.warn('⚠️ [Database Setup] chat_sessions table warning:', chatSessionsError.message);
    } else {
      console.log('✅ [Database Setup] chat_sessions table created');
    }

    // Create chat_messages table
    console.log('🔧 [Database Setup] Creating chat_messages table');
    ...
    const { error: chatMessagesError } = await supabase.rpc('exec_sql', { sql: chatMessagesSQL });
    if (chatMessagesError) {
      console.warn('⚠️ [Database Setup] chat_messages table warning:', chatMessagesError.message);
    } else {
      console.log('✅ [Database Setup] chat_messages table created');
    }
    ...
    return NextResponse.json({ 
      ...
      tables: ['user_agents', 'chat_sessions', 'chat_messages']
    });
