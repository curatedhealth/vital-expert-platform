...
export function DocumentsLibraryView() {
  ...
  const [sortBy, setSortBy] = useState<'date' | 'name' | 'source' | 'size' | 'year'>('date');
      ...
      if (data.success) {
        ...
        const enrichedDocuments = (data.documents || []).map((doc: any) => {
          ...
          const extractYear = (): number | undefined => {
            ...
            // Try to extract from filename (common patterns: 2024, 2023, etc.)
            const yearMatch = doc.file_name?.match(/\b(19|20)\d{2}\b/);
            if (yearMatch) return parseInt(yearMatch[0]);
            // Try from title
            const titleYearMatch = doc.title?.match(/\b(19|20)\d{2}\b/);
            if (titleYearMatch) return parseInt(titleYearMatch[0]);
            return undefined;
          };
  ...
  const inferDocumentType = (fileName?: string, title?: string): string => {
    ...
    if (text.match(/\b(research|study|paper|publication|journal)\b/)) return 'Research Paper';
    ...
    if (text.match(/\b(whitepaper|white paper)\b/)) return 'Whitepaper';
    
    return 'Document';
  };

  // Get unique values for filters
  const uniqueSources = Array.from(new Set(documents.map(d => d.source_name).filter(Boolean))).sort() as string[];
  const uniqueYears = Array.from(new Set(documents.map(d => d.year).filter(Boolean))).sort((a, b) => b! - a!);
  const uniqueTypes = Array.from(new Set(documents.map(d => d.document_type).filter(Boolean))).sort() as string[];
