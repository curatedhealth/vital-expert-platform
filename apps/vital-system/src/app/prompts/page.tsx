...
export default function PromptsPage() {
  ...
  const loadPrompts = async () => {
    try {
      setLoading(true);
      console.log('Prompts page: Loading prompts for current tenant...');
      ...
      console.log('Prompts page: Loaded', apiPrompts.length, 'prompts');

      // Transform API response to match frontend interface
      const transformedPrompts: Prompt[] = apiPrompts.map((p: any) => ({
        id: p.id,
        title: p.display_name || p.name,
        description: p.description || '',
        content: p.user_prompt_template || p.system_prompt || '',
        category: p.domain?.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()) || 'General',
        tags: p.tags || [],
        agent_type: p.target_users?.[0] || undefined,
        use_case: p.use_cases?.[0] || p.domain || 'General',
        difficulty_level: (p.complexity_level as 'beginner' | 'intermediate' | 'advanced') || 'intermediate',
        estimated_time_minutes: Math.ceil((p.estimated_tokens || 1000) / 50),
        popularity_score: 0,
        usage_count: 0,
        rating: 0,
        ...
        is_featured: false,
        is_public: p.is_active !== false,
        variables: extractVariables(p.user_prompt_template || ''),
      }));

      // If no prompts from API, fall back to mock data
      setPrompts(transformedPrompts.length > 0 ? transformedPrompts : mockPrompts);
    } catch (error) {
