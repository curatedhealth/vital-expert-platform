...
export function PersonaListItem({ persona, onClick }: PersonaListItemProps) {
  ...
  return (
    <Card className="cursor-pointer hover:bg-neutral-50 transition-colors" onClick={() => onClick?.(persona)}>
      <CardContent className="py-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-1 min-w-0">
            <UserCircle className="h-6 w-6 text-neutral-600 flex-shrink-0" />
            <div className="flex-1 min-w-0">
              ...
              {persona.title && (
                <p className="text-sm font-medium text-neutral-700 mb-1 truncate">{persona.title}</p>
              )}
              ...
              <div className="flex items-center gap-4 mt-2">
                ...
                {jtbdsCount > 0 && (
                  <div className="flex items-center gap-1 text-xs">
                    ...
                    <span className="font-semibold text-neutral-700">{jtbdsCount}</span>
                    <span className="text-neutral-500">JTBDs</span>
                  </div>
                )}
                {goalsCount > 0 && (
                  <div className="flex items-center gap-1 text-xs">
                    <TrendingUp className="h-3.5 w-3.5 text-green-500" />
                    <span className="font-semibold text-neutral-700">{goalsCount}</span>
                ...
                {challengesCount > 0 && (
                  <div className="flex items-center gap-1 text-xs">
                    ...
                    <span className="font-semibold text-neutral-700">{challengesCount}</span>
                    <span className="text-neutral-500">Challenges</span>
                  </div>
                )}
                {responsibilitiesCount > 0 && (
                  <div className="flex items-center gap-1 text-xs">
                    <Users className="h-3.5 w-3.5 text-purple-500" />
                    <span className="font-semibold text-neutral-700">{responsibilitiesCount}</span>
                    <span className="text-neutral-500">Responsibilities</span>
                  </div>
                )}
              </div>
            </div>
