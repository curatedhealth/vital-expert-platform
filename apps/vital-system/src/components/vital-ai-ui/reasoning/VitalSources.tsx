...
export type CitationStyle = 'default' | 'chicago' | 'apa' | 'mla';
...
export type VitalSourceProps = ComponentProps<'a'> & {
  ...
  /** Authors list for citation formatting */
  authors?: string[];
  /** Publication year */
  year?: number | string;
...
interface ChicagoCitationParts {
  authors?: string[];
  title?: string;
  journal?: string;
  volume?: string;
 ...
 * - Journal: Author Last, First. "Article Title." Journal Name Volume, no. Issue (Year): Pages.
 * - Book: Author Last, First. Title of Book. Place: Publisher, Year.
 * - Website: Author Last, First. "Page Title." Website Name. Access Date. URL.
 */
function formatChicagoCitation(parts: ChicagoCitationParts): ReactNode {
  const { authors, title, journal, volume, issue, year, pages, publisher, href, accessDate, doi, category } = parts;

  // Format authors: "Last, First, and Second Author"
  ...
  if (journal || category === 'Medical Literature') {
    return (
      <span className="text-sm leading-relaxed">
        ...
        {!doi && href && <span>. <span className="text-blue-600 break-all">{href}</span></span>}
        <span>.</span>
      </span>
    );
  }
  ...
  // Web source format
  if (category === 'Web' || (!journal && href)) {
    return (
      <span className="text-sm leading-relaxed">
        {authorStr && <span>{authorStr} </span>}
        {title && <span>"{title}." </span>}
        {publisher && <span><em>{publisher}</em>. </span>}
        {accessDate && <span>Accessed {accessDate}. </span>}
        {href && <span className="text-blue-600 break-all">{href}</span>}
        <span>.</span>
      </span>
    );
  }

  // Book/document format (default)
  return (
    <span className="text-sm leading-relaxed">
      {authorStr && <span>{authorStr} </span>}
      {title && <em>{title}. </em>}
      ...
      {year && <span>{year}</span>}
      {href && <span>. <span className="text-blue-600 break-all">{href}</span></span>}
      <span>.</span>
    </span>
  );
}
...
/**
 * Individual source with VITAL metadata and Chicago style citation support
 */
    ...
    if (citationStyle === 'chicago') {
      return (
        <a
          ...
          rel="noreferrer"
          className={cn(
            'block p-3 rounded-lg border bg-card',
        ...
        >
          {children ?? (
            <div className="space-y-2">
              ...
              {/* Snippet/excerpt */}
              {snippet && (
                <p className="text-xs text-muted-foreground line-clamp-2 mt-2 italic border-l-2 border-slate-200 pl-2">
                  "{snippet}"
                </p>
              )}
            </div>
...
export { retrieverIcons, retrieverColors, retrieverLabels, categoryIcons, formatChicagoCitation };
