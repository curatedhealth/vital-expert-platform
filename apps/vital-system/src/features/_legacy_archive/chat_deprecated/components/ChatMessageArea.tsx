...
export function ChatMessageArea({
  ...
  return (
    <div className="flex flex-col h-full bg-canvas-surface">
      {/* Expert Profile Header - Show when in manual mode */}
      {interactionMode === 'manual' && selectedExpert && (
        <div className="border-b px-6 py-3 bg-neutral-50 flex-shrink-0">
          <div className="max-w-4xl mx-auto flex items-center gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center bg-canvas-surface border-2 border-neutral-200 overflow-hidden flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-neutral-600 truncate">{selectedExpert.description}</p>
            </div>
            {onChangeExpert && (
              <Button
                variant="ghost"
      ...
      {messages.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {promptStarters.slice(0, 4).map((prompt, index) => (
                  <button
                    ...
                    className="p-4 text-left border-2 border-neutral-200 rounded-xl hover:border-blue-300 hover:bg-blue-50/50 transition-all duration-200 group"
                  >
                    <div className="text-sm text-neutral-700 group-hover:text-neutral-900 line-clamp-3 font-medium">
                      {prompt.text}
                    </div>
                  </button>
                ))}
        ...
        <>
          ...
          {recommendedAgents.length > 0 && (
            <div className="absolute inset-0 bg-canvas-surface/95 backdrop-blur-sm z-50 flex items-center justify-center p-6">
                    ...
                    >
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-neutral-500 italic">{agent.reasoning}</p>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="flex-shrink-0 border-t bg-canvas-surface">
