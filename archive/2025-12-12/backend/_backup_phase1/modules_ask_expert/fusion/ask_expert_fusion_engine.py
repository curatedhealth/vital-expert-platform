"""
VITAL Path AI Services - Ask Expert Fusion Engine
...
Naming Convention:
- Class: AskExpertFusionEngine
- Methods: retrieve, explain_selection
- Logs: ask_expert_fusion_engine_{action}
...
class AskExpertFusionResult:
    ...
    vector_results: List[AskExpertRankedItem] = field(default_factory=list)
    graph_results: List[AskExpertRankedItem] = field(default_factory=list)
    relational_results: List[AskExpertRankedItem] = field(default_factory=list)
...
class AskExpertFusionEngine:
    """
    Ask Expert Fusion Intelligence Engine.
    ...
    def __init__(
        self,
        vector_retriever: Optional[AskExpertVectorRetriever] = None,
        graph_retriever: Optional[AskExpertGraphRetriever] = None,
        relational_retriever: Optional[AskExpertRelationalRetriever] = None,
        ...
        logger.info(
            "ask_expert_fusion_engine_initialized",
    ...
    async def retrieve(
        ...
        """
        logger.info(
            "ask_expert_fusion_engine_retrieve_started",
            tenant_id=tenant_id,
            top_k=top_k,
        ...
        try:
            ...
            if not tasks:
                logger.warning("ask_expert_fusion_engine_no_retrievers")
            ...
            try:
                ...
                logger.warning(
                    "ask_expert_fusion_engine_timeout",
                    timeout=self.timeout,
                )
            ...
            for name, res in zip(task_names, results):
                if isinstance(res, Exception):
                    logger.error(
                        f"ask_expert_fusion_engine_{name}_failed",
                        error=str(res),
                    )
            ...
            if ranked_lists:
                result.fused_rankings = ask_expert_weighted_rrf(
                    ranked_lists,
                    weights=self.weights,
                    k=60,
                )[:top_k]
            ...
            logger.info(
                "ask_expert_fusion_engine_retrieve_completed",
                tenant_id=tenant_id,
            ...
            logger.error(
                "ask_expert_fusion_engine_retrieve_failed",
                tenant_id=tenant_id,
                error=str(e),
            )
    ...
    async def _safe_retrieve(
        ...
        try:
            ...
            logger.error(
                "ask_expert_fusion_engine_retriever_error",
                error=str(e),
            )
            return []
    ...
    def explain_selection(
        self,
        agent_id: str,
        fusion_result: AskExpertFusionResult,
    ) -> Dict[str, Any]:
        """
        ...
        explanation = ask_expert_explain_rrf_score(agent_id, agent_data)
...
def create_ask_expert_fusion_engine(
    ...
    Returns:
        Configured AskExpertFusionEngine
    """
    ...
    if supabase_client:
        vector_retriever = AskExpertVectorRetriever(
            supabase_client=supabase_client,
            embedding_service=embedding_service,
        )
        relational_retriever = AskExpertRelationalRetriever(
            supabase_client=supabase_client,
        )
    
    if neo4j_driver:
        graph_retriever = AskExpertGraphRetriever(
            neo4j_driver=neo4j_driver,
        )
    
    return AskExpertFusionEngine(
