...
class AskExpertRequest(BaseModel):
    ...
    # 4-Mode flags (aligned with PRD/ARD v8)
    is_automatic: bool = Field(False, description="Auto-select experts (true) or manual selection (false)")
    is_autonomous: bool = Field(False, description="Autonomous execution (true) or interactive (false)")
    ...
    selected_agent_ids: List[str] = Field(default_factory=list, description="Explicitly selected agent IDs (for manual modes)")
    context: Optional[Dict[str, Any]] = Field(default=None, description="Execution context / payload")
    max_response_tokens: Optional[int] = Field(default=None, description="Optional response token limit")
    hitl_enabled: bool = Field(default=False, description="Enable HITL checkpoints (recommended for autonomous modes)")
    hitl_safety_level: Optional[str] = Field(default=None, description="HITL safety profile (e.g., low/medium/high)")
...
class AskExpertResponse(BaseModel):
    ...
    agent_level: Optional[str] = Field(None, description="Agent level (L1-L5)")
                ...
                "modes": [
                    {
                        ...
                        "description": "User picks an L2 expert; chat with streaming",
                        "selection": "manual",
                        "autonomy": "interactive",
                        "response_time": "0.2-2s",
                        "requires_agent_selection": True,
                        "requires_l5_tools": True
                    }
                ]
            }
        }
...
def normalize_mode_flags(request: AskExpertRequest) -> None:
    """
    Keep compatibility between string-based mode and boolean flags.
    If the client sends `mode`, set the flags accordingly.
    """
    mode_to_flags = {
        "mode1": (False, False),
        "mode2": (True, False),
    ...
    if request.mode in mode_to_flags:
        request.is_automatic, request.is_autonomous = mode_to_flags[request.mode]


async def validate_request(request: AskExpertRequest) -> None:
    """Validate request based on mode requirements"""
    normalize_mode_flags(request)
    ...
    **Modes**:
    ...
    try:
        ...
        response = AskExpertResponse(
            ...
            agent_level=result.get("agent_level") or result.get("level") or result.get("tier"),
    ...
    modes = [
        {
            "mode_id": "mode1",
            "name": "Manual-Interactive",
            "description": "User picks an L2 Expert; streaming chat with required L5 tools.",
            "selection": "manual",
            "autonomy": "interactive",
            "response_time": "0.2-2s",
            "requires_agent_selection": True,
            "requires_l5_tools": True,
            "supports_hitl": False,
            "use_cases": ["Specific expert consultation", "Focused domain questions", "Rapid guidance"]
        },
        {
            "mode_id": "mode2",
            "name": "Auto-Interactive",
            "description": "AI selects best L2/L3 experts via fusion; streaming chat with L5 tools.",
            "selection": "automatic",
            "autonomy": "interactive",
            "response_time": "0.3-3s",
        ...
        },
        {
            ...
            "description": "User picks experts; deep work with HITL checkpoints; L4 workers + L5 tools required.",
            "selection": "manual",
            "autonomy": "autonomous",
            "response_time": "1-15min (streaming)",
            "requires_agent_selection": True,
            "requires_l5_tools": True,
            "supports_hitl": True,
            "use_cases": ["Complex analysis with known expert", "Strategic planning", "Evidence-backed research"]
        },
        {
            "mode_id": "mode4",
            "name": "Auto-Autonomous",
            "description": "L1 orchestrates team selection + execution; fusion selection; L4/L5 mandatory; HITL supported.",
            "selection": "automatic",
            "autonomy": "autonomous",
            "response_time": "5-25min (streaming)",
            "requires_agent_selection": False,
            "requires_l5_tools": True,
            "supports_hitl": True,
