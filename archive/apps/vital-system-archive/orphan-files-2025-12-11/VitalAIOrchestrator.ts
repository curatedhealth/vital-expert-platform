...
import { agentRAGIntegration, AgentRAGQuery } from '@/lib/services/rag/agent-rag-integration';
import {
  ExecutionContext
  ...
  async processQuery(
    ...
    try {
      ...
      performanceMetricsService.endOperation(
        operationId,
        true,
        { session_id: context.session_id || 'unknown', user_id: context.user_id || 'unknown' }
      );
    ...
    try {
      ...
      try {
        const ragQuery: AgentRAGQuery = {
          ...
          topK: Math.min(5, Math.ceil(intent.complexity * 6))
        };
        ...
        if (ragResponse && ragResponse.sources && ragResponse.sources.length > 0) {
          ...
          ragMetadata = {
            ragEnhanced: true,
            ragSystemsUsed: (ragResponse as any).ragSystemsUsed || [],
            knowledgeSourcesCount: ragResponse.sources.length,
            ragConfidence: ragResponse.confidence,
            agentKnowledgeDomains: (ragResponse as any).agentContext?.knowledgeDomains || []
          };
        }
      } catch (ragError) {
