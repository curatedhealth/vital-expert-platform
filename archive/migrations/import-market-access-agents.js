...
async function importMarketAccessAgents() {
  console.log('💰 IMPORTING MARKET ACCESS AGENTS TO SUPABASE CLOUD\n');
  console.log('=' .repeat(70));

  try {
    // Read the Market Access agents JSON file
    const jsonPath = path.join(__dirname, 'docs', 'MARKET_ACCESS_AGENTS_30_COMPLETE.json');
    
    if (!fs.existsSync(jsonPath)) {
      throw new Error(`Market Access agents file not found: ${jsonPath}`);
    }

    const agentsData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
    console.log(`📋 Found ${agentsData.agents.length} Market Access agents to import`);
    ...
    for (const agent of agentsData.agents) {
      try {
        ...
        const agentRecord = {
          ...
          target_users: ['market_access', 'payer_relations', 'health_economics'],
          compliance_tags: ['fda', 'hipaa', 'payer_compliance'],
    ...
    console.log('\n' + '=' .repeat(70));
    console.log('🎉 MARKET ACCESS AGENTS IMPORT SUMMARY');
    console.log('=' .repeat(70));
    ...
    console.log(`\n📊 Total Market Access agents in database: ${totalAgents}`);
