...
SELECT 
  ...
  COALESCE(
    (SELECT tier FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
    1
  ) AS tier,
  CASE 
    WHEN COALESCE(
      (SELECT tier FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
      1
    ) = 1 THEN 'Core / High Authority'
    WHEN COALESCE(
      (SELECT tier FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
      1
    ) = 2 THEN 'Specialized / High Value'
    ELSE 'Emerging / Future-Focused'
  END AS tier_label,
  COALESCE(
    (SELECT priority FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
    11
  ) AS priority,
  'Established'::maturity_level AS maturity_level,
  'High'::exposure_level AS regulatory_exposure,
  'Medium'::exposure_level AS pii_sensitivity,
  'text-embedding-3-large' AS embedding_model,
  0.95 AS rag_priority_weight,  -- High priority for authoritative content
  'enterprise_confidential'::access_policy_level AS access_policy,  -- SECURED: Require enterprise access
  ...
  -- Legacy fields from existing domain (handle missing columns gracefully)
  COALESCE(
    (SELECT code FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
    'DIGITAL_HEALTH'
  ) AS code,
  'digital_health' AS slug,
  COALESCE(
    (SELECT name FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
    'Digital Health'
  ) AS name,
  COALESCE(
    (SELECT description FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
    'Digital health solutions and technology'
  ) AS description,
  COALESCE(
    (SELECT keywords FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
    ARRAY['digital_health', 'samd', 'dtx', 'telemedicine']
  ) AS keywords,
  COALESCE(
    (SELECT is_active FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
    true
  ) AS is_active,
  COALESCE(
    (SELECT color FROM public.knowledge_domains WHERE slug = 'digital_health' LIMIT 1),
    '#10B981'
  ) AS color,
  -- Icon column: use default if column doesn't exist in old table
  'smartphone' AS icon
FROM (SELECT 1) existing
...
SELECT 
  ...
  'global' AS domain_scope,
  COALESCE(
    (SELECT tier FROM public.knowledge_domains WHERE slug = 'regulatory_affairs' LIMIT 1),
    1
  ) AS tier,
  CASE 
    WHEN COALESCE(
  ...
  END AS tier_label,
  COALESCE(
    (SELECT priority FROM public.knowledge_domains WHERE slug = 'regulatory_affairs' LIMIT 1),
    1
  ) AS priority,
  'Established'::maturity_level AS maturity_level,
  'High'::exposure_level AS regulatory_exposure,
  'Low'::exposure_level AS pii_sensitivity,
  'text-embedding-3-large' AS embedding_model,
  0.95 AS rag_priority_weight,  -- High priority for authoritative content
  'enterprise_confidential'::access_policy_level AS access_policy,  -- SECURED: Require enterprise access
  ...
  -- Legacy fields from existing domain (handle missing columns gracefully)
  COALESCE(
    (SELECT code FROM public.knowledge_domains WHERE slug = 'regulatory_affairs' LIMIT 1),
    'REG_AFFAIRS'
  ) AS code,
  'regulatory_affairs' AS slug,
  COALESCE(
    (SELECT name FROM public.knowledge_domains WHERE slug = 'regulatory_affairs' LIMIT 1),
    'Regulatory Affairs'
  ) AS name,
  COALESCE(
    (SELECT description FROM public.knowledge_domains WHERE slug = 'regulatory_affairs' LIMIT 1),
    'Regulatory compliance and submissions'
  ) AS description,
  COALESCE(
    (SELECT keywords FROM public.knowledge_domains WHERE slug = 'regulatory_affairs' LIMIT 1),
    ARRAY['regulatory', 'fda', 'ema', 'submissions', 'compliance']
  ) AS keywords,
  COALESCE(
    (SELECT is_active FROM public.knowledge_domains WHERE slug = 'regulatory_affairs' LIMIT 1),
    true
  ) AS is_active,
  COALESCE(
    (SELECT color FROM public.knowledge_domains WHERE slug = 'regulatory_affairs' LIMIT 1),
    '#EF4444'
  ) AS color,
  -- Icon column: use default if column doesn't exist in old table
  'file-check' AS icon
