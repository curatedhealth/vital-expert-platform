-- =====================================================================================
-- UC_RA_001: FDA Software Classification (SaMD) - COMPLETE SEED FILE
-- =====================================================================================
-- Purpose: Determine if a digital health product qualifies as FDA Software as a 
--          Medical Device (SaMD) and identify the appropriate regulatory pathway
-- 
...
--   3. Task Definitions (6 tasks)
--   4. Task Dependencies (sequential workflow)
--   5. Agent Assignments (11 assignments)
--   6. Persona Assignments (7 assignments)
--   7. Tool Assignments (3 assignments)
--   8. RAG Source Assignments (5 assignments)
--   9. Verification Query
-- =====================================================================================

-- =====================================================================================
-- SETUP: Create session_config for tenant lookup
-- =====================================================================================
...
DECLARE
  v_tenant_id UUID;
BEGIN
  -- Create temporary table for tenant context
  ...
  IF v_tenant_id IS NULL THEN
    RAISE EXCEPTION 'Tenant "digital-health-startup" not found. Please check tenants table.';
  END IF;
...
FROM session_config sc
CROSS JOIN (VALUES
  ('TSK-RA-001-01', 'TSK-RA-001-01', 'Analyze Product Description & Intended Use', 'Extract and structure product features, intended use statement, target users, and clinical claims', 1, '{"complexity": "INTERMEDIATE", "duration_minutes": 15, "deliverable": "Structured product summary"}'::jsonb),
  ('TSK-RA-001-02', 'TSK-RA-001-02', 'Assess FD&C Act Section 201(h) Device Definition', 'Determine if product meets legal definition of "device" under Federal Food, Drug, and Cosmetic Act', 2, '{"complexity": "ADVANCED", "duration_minutes": 15, "deliverable": "Device definition assessment", "key_question": "Does it diagnose, cure, mitigate, treat, or prevent disease?"}'::jsonb),
  ('TSK-RA-001-03', 'TSK-RA-001-03', 'Apply FDA Enforcement Discretion Criteria', 'Check if product falls under FDA enforcement discretion per 2019 guidance (low risk wellness)', 3, '{"complexity": "INTERMEDIATE", "duration_minutes": 15, "deliverable": "Enforcement discretion determination", "guidance": "Policy for Device Software Functions (2019)"}'::jsonb),
  ('TSK-RA-001-04', 'TSK-RA-001-04', 'Determine Risk Level & Device Class', 'Assess risk level (serious injury/death potential) and determine device class (I, II, or III)', 4, '{"complexity": "ADVANCED", "duration_minutes": 20, "deliverable": "Risk assessment & class determination"}'::jsonb),
  ('TSK-RA-001-05', 'TSK-RA-001-05', 'Recommend Regulatory Pathway', 'Based on classification, recommend specific regulatory pathway (510(k), De Novo, PMA, or exempt)', 5, '{"complexity": "ADVANCED", "duration_minutes": 15, "deliverable": "Pathway recommendation with rationale"}'::jsonb),
  ('TSK-RA-001-06', 'TSK-RA-001-06', 'Generate Classification Report', 'Compile comprehensive classification report with citations, precedent analysis, and recommendations', 6, '{"complexity": "INTERMEDIATE", "duration_minutes": 10, "deliverable": "FDA SaMD Classification Report (5-8 pages)"}'::jsonb)
...
FROM session_config sc
CROSS JOIN (VALUES
  ('TSK-RA-001-02', 'TSK-RA-001-01'),
  ('TSK-RA-001-03', 'TSK-RA-001-02'),
  ('TSK-RA-001-04', 'TSK-RA-001-03'),
  ('TSK-RA-001-05', 'TSK-RA-001-04'),
  ('TSK-RA-001-06', 'TSK-RA-001-05')
...
-- =====================================================================================
-- SECTION 6: PERSONA ASSIGNMENTS (Human-in-the-Loop)
-- =====================================================================================
...
FROM session_config sc
CROSS JOIN (VALUES
  ('TSK-RA-001-02', 'RAG-FDA-GUIDANCE', 'FD&C Act Section 201(h) device definition, Software as Medical Device guidance'),
  ('TSK-RA-001-03', 'RAG-FDA-DIGITAL-HEALTH', 'FDA enforcement discretion policy 2019, wellness app guidance, low-risk device guidance'),
  ('TSK-RA-001-04', 'RAG-FDA-GUIDANCE', 'Device classification by risk level, Class I II III criteria, 510(k) exemption criteria'),
  ('TSK-RA-001-05', 'RAG-FDA-GUIDANCE', '510(k) requirements, De Novo pathway, PMA requirements, device classification regulation'),
  ('TSK-RA-001-05', 'RAG-FDA-DIGITAL-HEALTH', 'DTx regulatory precedents, digital therapeutics pathways, SaMD classification examples')
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'UC_RA_001: FDA Software Classification';
  RAISE NOTICE '========================================';
  ...
  RAISE NOTICE '========================================';
  RAISE NOTICE '✓ UC_RA_001 seeded successfully!';
  RAISE NOTICE '';
END $$;
