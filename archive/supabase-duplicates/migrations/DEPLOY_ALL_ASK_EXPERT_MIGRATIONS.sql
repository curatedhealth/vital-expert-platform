...
CREATE POLICY IF NOT EXISTS "user_agents_select_own"
  ON user_agents FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "user_agents_insert_own"
  ON user_agents FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "user_agents_delete_own"
  ON user_agents FOR DELETE
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "service_role_all_user_agents"
