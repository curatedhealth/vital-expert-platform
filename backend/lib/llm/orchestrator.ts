...
class LLMOrchestrator {
  ...
  async query(
    ...
    try {
      ...
      if (config.provider === 'openai') {
        const result = await this.queryOpenAI(question, context, config, options);
        response = result.content;
        tokensUsed = result.tokensUsed;
      } else {
        const result = await this.queryAnthropic(question, context, config, options);
        response = result.content;
        tokensUsed = result.tokensUsed;
      }

      // Extract citations if present
      const citations = this.extractCitations(response, options.citations);

      // Calculate confidence score
      const confidence = this.calculateConfidence(response, citations, modelType);
  ...
  private async queryAnthropic(
    ...
    const message = await this.anthropic.completions.create({
      model: config.model,
    ...
    const responseContent = message.completion || '';

    // Anthropic doesn't provide token usage in the same format
    const tokensUsed = this.estimateTokens(content + responseContent);
  ...
  private analyzeConsensus(responses: LLMResponse[]): ConsensusAnalysis {
    ...
    const agreementScore = this.calculateAgreement(responses);
