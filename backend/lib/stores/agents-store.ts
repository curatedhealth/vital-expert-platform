...
const convertDbAgentToStoreFormat = (dbAgent: DbAgent): Agent => {
  return {
    ...
    is_public: false, // Default value since field doesn't exist in current schema
    status: dbAgent.status as 'active' | 'inactive' | 'deprecated',
    tier: dbAgent.tier ?? 1,
    priority: dbAgent.priority ?? 100,
    ...
    business_function: dbAgent.business_function || undefined,
    role: dbAgent.role || undefined,
    domain: undefined, // Field doesn't exist in current schema
...
const convertStoreAgentToDbFormat = (storeAgent: Agent): Partial<DbAgent> => {
  return {
    ...
    status: storeAgent.status === 'inactive' ? 'deprecated' : storeAgent.status as 'active' | 'development' | 'testing' | 'deprecated',
