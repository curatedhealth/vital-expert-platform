...
\restrict Wpjsrsm5f8Laond3pWfP3XPWoTFgLbrxmAk8L7cjQSrxA9b2ClHjH16uwv9GexI
...
CREATE TABLE public.agent_capabilities (
    ...
    CONSTRAINT agent_capabilities_proficiency_level_check CHECK (((proficiency_level)::text = ANY (ARRAY[('basic'::character varying)::text, ('intermediate'::character varying)::text, ('advanced'::character varying)::text, ('expert'::character varying)::text])))
);
...
\unrestrict Wpjsrsm5f8Laond3pWfP3XPWoTFgLbrxmAk8L7cjQSrxA9b2ClHjH16uwv9GexI
