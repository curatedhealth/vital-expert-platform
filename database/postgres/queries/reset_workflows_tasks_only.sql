...
BEGIN
  ...
  -- New workflow model child tables (bottom-up, with existence checks)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'workflow_task_pain_points') THEN
    DELETE FROM workflow_task_pain_points;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from workflow_task_pain_points', row_count;
  ELSE
    RAISE NOTICE '⊗ Table workflow_task_pain_points does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'workflow_task_data_requirements') THEN
    DELETE FROM workflow_task_data_requirements;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from workflow_task_data_requirements', row_count;
  ELSE
    RAISE NOTICE '⊗ Table workflow_task_data_requirements does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'workflow_task_skills') THEN
    DELETE FROM workflow_task_skills;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from workflow_task_skills', row_count;
  ELSE
    RAISE NOTICE '⊗ Table workflow_task_skills does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'workflow_task_tools') THEN
    DELETE FROM workflow_task_tools;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from workflow_task_tools', row_count;
  ELSE
    RAISE NOTICE '⊗ Table workflow_task_tools does not exist, skipping';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⊗ Table workflow_tasks does not exist, skipping';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⊗ Table workflow_stages does not exist, skipping';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⊗ Table workflow_templates does not exist, skipping';
  END IF;
END $$;
...
BEGIN
  ...
  -- Legacy JTBD workflow model child tables (bottom-up, with existence checks)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'jtbd_activity_pain_points') THEN
    DELETE FROM jtbd_activity_pain_points;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from jtbd_activity_pain_points', row_count;
  ELSE
    RAISE NOTICE '⊗ Table jtbd_activity_pain_points does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'jtbd_activity_required_data') THEN
    DELETE FROM jtbd_activity_required_data;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from jtbd_activity_required_data', row_count;
  ELSE
    RAISE NOTICE '⊗ Table jtbd_activity_required_data does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'jtbd_activity_required_tools') THEN
    DELETE FROM jtbd_activity_required_tools;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from jtbd_activity_required_tools', row_count;
  ELSE
    RAISE NOTICE '⊗ Table jtbd_activity_required_tools does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'jtbd_activity_required_skills') THEN
    DELETE FROM jtbd_activity_required_skills;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from jtbd_activity_required_skills', row_count;
  ELSE
    RAISE NOTICE '⊗ Table jtbd_activity_required_skills does not exist, skipping';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⊗ Table jtbd_workflow_activities does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'jtbd_stage_pain_points') THEN
    DELETE FROM jtbd_stage_pain_points;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from jtbd_stage_pain_points', row_count;
  ELSE
    RAISE NOTICE '⊗ Table jtbd_stage_pain_points does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'jtbd_stage_key_activities') THEN
    DELETE FROM jtbd_stage_key_activities;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from jtbd_stage_key_activities', row_count;
  ELSE
    RAISE NOTICE '⊗ Table jtbd_stage_key_activities does not exist, skipping';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⊗ Table jtbd_workflow_stages does not exist, skipping';
  END IF;
END $$;
...
BEGIN
  ...
  -- Task child tables (bottom-up, with existence checks)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'step_parameters') THEN
    DELETE FROM step_parameters;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from step_parameters', row_count;
  ELSE
    RAISE NOTICE '⊗ Table step_parameters does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_steps') THEN
    DELETE FROM task_steps;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_steps', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_steps does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_input_definitions') THEN
    DELETE FROM task_input_definitions;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_input_definitions', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_input_definitions does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_output_definitions') THEN
    DELETE FROM task_output_definitions;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_output_definitions', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_output_definitions does not exist, skipping';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_prerequisites') THEN
    DELETE FROM task_prerequisites;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_prerequisites', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_prerequisites does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_agents') THEN
    DELETE FROM task_agents;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_agents', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_agents does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_tools') THEN
    DELETE FROM task_tools;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_tools', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_tools does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_skills') THEN
    DELETE FROM task_skills;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_skills', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_skills does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_inputs') THEN
    DELETE FROM task_inputs;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_inputs', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_inputs does not exist, skipping';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_outputs') THEN
    DELETE FROM task_outputs;
    GET DIAGNOSTICS row_count = ROW_COUNT;
    RAISE NOTICE '✓ Deleted % rows from task_outputs', row_count;
  ELSE
    RAISE NOTICE '⊗ Table task_outputs does not exist, skipping';
  END IF;
