...
export class AgentTools {
    ...
    parseAgentFromCode(content, filePath) {
    ...
    parseAgentFromJSON(jsonData, filePath) {
        return {
    ...
    async listAllAgents(tier, businessFunction, status) {
        let agents = Array.from(this.agentsCache.values());
        // Apply filters
        if (tier) {
            agents = agents.filter(agent => agent.tier === parseInt(tier) ||
                agent.tier?.toString() === tier ||
                (tier === 'core' && !agent.tier));
        }
        if (businessFunction) {
            agents = agents.filter(agent => agent.businessFunction?.toLowerCase().includes(businessFunction.toLowerCase()));
        }
        if (status) {
            agents = agents.filter(agent => agent.status?.toLowerCase() === status.toLowerCase());
        }
        return {
    ...
    async getAgentDetails(agentId) {
        const agent = this.agentsCache.get(agentId.toLowerCase()) ||
            Array.from(this.agentsCache.values()).find(a => a.name.toLowerCase() === agentId.toLowerCase() ||
    ...
    async searchAgents(query, field = 'all') {
        const results = [];
        ...
        for (const agent of this.agentsCache.values()) {
            ...
            if (field === 'all' || field === 'capabilities') {
                if (agent.capabilities.some((cap) => cap.toLowerCase().includes(searchQuery))) {
                    matches = true;
    ...
    async listAgentCapabilities() {
        ...
        const capabilities = Array.from(allCapabilities).map((capability) => ({
            capability,
