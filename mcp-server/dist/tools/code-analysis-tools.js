...
export class CodeAnalysisTools {
    ...
    async findDependencies(filePath, includeTransitive = false) {
        const analysis = await this.analyzeFile(filePath);
        const dependencies = analysis.imports.map((imp) => imp.module);
    ...
    async searchPatterns(pattern, filePattern, caseSensitive) {
    ...
    async getFunctionInfo(filePath, functionName) {
        ...
        if (functionName) {
            functions = functions.filter((fn) => fn.name === functionName);
        }
        return {
            filePath,
    ...
    async findDuplicates(minLines, filePattern) {
