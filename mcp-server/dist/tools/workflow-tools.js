...
export class WorkflowTools {
    ...
    calculateComplexity(nodeCount, edgeCount) {
    ...
    async listWorkflows(type, status) {
        let workflows = Array.from(this.workflowsCache.values());
        // Apply filters
        if (type) {
            workflows = workflows.filter(workflow => workflow.type?.toLowerCase().includes(type.toLowerCase()));
        }
        if (status) {
            workflows = workflows.filter(workflow => workflow.status?.toLowerCase() === status.toLowerCase());
        }
        return {
    ...
    async getWorkflowDetails(workflowId) {
        const workflow = this.workflowsCache.get(workflowId.toLowerCase()) ||
            Array.from(this.workflowsCache.values()).find(w => w.name.toLowerCase() === workflowId.toLowerCase() ||
    ...
    getNodeType(nodeName) {
        if (nodeName.includes('Start') || nodeName.includes('Init'))
            return 'start';
        if (nodeName.includes('End') || nodeName.includes('Finish'))
            return 'end';
        if (nodeName.includes('Decision') || nodeName.includes('Condition'))
            return 'decision';
        if (nodeName.includes('Process') || nodeName.includes('Action'))
            return 'process';
        if (nodeName.includes('Agent') || nodeName.includes('LLM'))
            return 'agent';
        return 'default';
    }
    async searchWorkflows(query) {
        ...
        for (const workflow of this.workflowsCache.values()) {
            ...
            if (workflow.nodes?.some((node) => node.toLowerCase().includes(searchQuery))) {
                matches = true;
    ...
    getComplexityRecommendations(complexity, density) {
