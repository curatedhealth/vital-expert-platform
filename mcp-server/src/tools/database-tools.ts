...
export class DatabaseTools {
  ...
  private async loadDatabaseStructure(): Promise<void> {
  ...
  private async parseMigrationFile(filePath: string): Promise<any> {
  ...
  private async parseSchemaFile(filePath: string): Promise<any> {
    const content = await fs.readFile(path.join(this.projectRoot, filePath), 'utf8');
    const fileName = path.basename(filePath);
  ...
  private analyzeMigrationContent(content: string): any {
    const analysis = {
      operations: [] as string[],
      tablesAffected: new Set<string>(),
      isDestructive: false,
  ...
  private extractColumns(tableDefinition: string): any[] {
    const columns = [];
  ...
  private extractColumnType(columnDef: string): string {
    const typeMatch = columnDef.match(/(\w+)/);
    return typeMatch ? typeMatch[1] : 'unknown';
  }

  private extractConstraints(tableDefinition: string): any[] {
    const constraints = [];
  ...
  private extractTableName(foreignKeyDef: string): string {
  ...
  private async listMigrations(status?: string, dateRange?: string): Promise<any> {
    let migrations = Array.from(this.migrationsCache.values());

    // Apply filters
    if (status) {
      migrations = migrations.filter(migration => 
        migration.analysis?.operations?.includes(status.toUpperCase())
      );
    }
    ...
    if (dateRange) {
      ...
      if (startDate && endDate) {
        migrations = migrations.filter(migration => {
          if (!migration.createdAt) return false;
  ...
  private async getMigrationDetails(migrationId: string): Promise<any> {
    const migration = this.migrationsCache.get(migrationId) || 
                    Array.from(this.migrationsCache.values()).find(m => 
                      m.fileName.includes(migrationId) ||
                      m.id === migrationId
                    );
  ...
  private async getSchemaOverview(table?: string): Promise<any> {
  ...
  private async searchSchema(query: string, type?: string): Promise<any> {
    const schemas = Array.from(this.schemaCache.values());
    const results = [];
  ...
  private async getRelationships(tableName?: string): Promise<any> {
    const schemas = Array.from(this.schemaCache.values());
    const allRelationships = schemas.flatMap(schema => schema.relationships || []);
    
    let relationships = allRelationships;
    if (tableName) {
      relationships = allRelationships.filter(rel => 
        rel.fromTable.toLowerCase() === tableName.toLowerCase() ||
  ...
  private async analyzeSchemaChanges(since?: string): Promise<any> {
    ...
    if (since) {
      ...
      filteredMigrations = migrations.filter(migration => 
        migration.createdAt && migration.createdAt >= sinceDate
      );
    }
