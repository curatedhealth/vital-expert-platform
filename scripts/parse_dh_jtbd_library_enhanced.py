#!/usr/bin/env python3
"""
Enhanced Digital Health JTBD Library Parser
Extracts complete persona profiles with all nested details
Generates comprehensive JSON and SQL files for database import
"""
...
def parse_yaml_list(lines: List[str], start_idx: int) -> tuple[List[str], int]:
    """Parse a YAML list starting from start_idx, return (list, next_idx)"""
    items = []
    idx = start_idx
    ...
    while idx < len(lines):
        ...
        # If it's indented continuation
            ...
            items[-1] += ' ' + stripped
            idx += 1
        # If we hit a new key or blank line, stop
...
def parse_persona_profile_complete(profile_section: str) -> Dict[str, Any]:
    """Parse complete persona profile including nested lists"""
    profile = {}
    ...
    while idx < len(lines):
        ...
        if ':' in stripped and not stripped.startswith('-'):
            ...
            if idx + 1 < len(lines) and lines[idx + 1].strip().startswith('-'):
                # Parse the list
                list_items, next_idx = parse_yaml_list(lines, idx + 1)
                profile[key] = list_items
                idx = next_idx
            elif value:
                # Simple key-value
                profile[key] = value
...
def extract_persona_sections(content: str) -> List[Dict[str, Any]]:
    """Extract all persona sections from the markdown with complete details"""
    personas = []
    ...
    for match in matches:
        ...
        # Parse profile with complete nested structure
        profile = parse_persona_profile_complete(profile_section)
...
def generate_json_output(personas: List[Dict[str, Any]], output_path: Path):
    ...
    output_data = {
        "metadata": {
            ...
            "parser_version": "2.0 - Enhanced with complete profile capture"
        },
        "personas": []
    }
    ...
    print(f"   - {output_data['metadata']['total_personas']} personas with complete profiles")
    print(f"   - {output_data['metadata']['total_jobs']} jobs")
    print(f"   - Responsibilities, pain_points, and other nested fields captured")
...
def main():
    ...
    print("=" * 80)
    print("Digital Health JTBD Library Parser v2.0 (Enhanced)")
    print("=" * 80)
    print()
    ...
    # Extract personas
    print("🔍 Extracting personas and JTBDs with complete profiles...")
    personas = extract_persona_sections(content)
    
    total_jobs = sum(len(p['jobs']) for p in personas)
    
    # Count personas with responsibilities and pain_points
    with_responsibilities = sum(1 for p in personas if 'responsibilities' in p['profile'])
    with_pain_points = sum(1 for p in personas if 'pain_points' in p['profile'])
    
    print(f"✅ Extracted:")
    print(f"   - {len(personas)} personas")
    print(f"   - {total_jobs} jobs-to-be-done")
    print(f"   - {with_responsibilities} personas with responsibilities")
    print(f"   - {with_pain_points} personas with pain_points")
    print()
    ...
    json_output = output_dir / f"dh_jtbd_library_enhanced_{timestamp}.json"
    sql_output = output_dir / f"dh_jtbd_library_enhanced_{timestamp}.sql"
    
    print("📝 Generating enhanced output files...")
    print()
    ...
    print("=" * 80)
    print("✨ SUCCESS! Enhanced files generated:")
    print(f"   📄 JSON: {json_output}")
    print(f"   📄 SQL:  {sql_output}")
    print("=" * 80)
    print()
    print("✅ Enhanced Features:")
    print("   - Complete persona profiles with responsibilities")
    print("   - Pain points and contextual details captured")
    print("   - All nested lists and attributes preserved")
    print("   - Ready for comprehensive persona database")
    print()
    print("Next steps:")
    print("1. Review the enhanced JSON file for complete persona data")
    print("2. Import JTBDs to Supabase using the SQL file")
    print("3. Create persona records with full profile details")
    print("4. Map JTBDs to personas using jtbd_org_persona_mapping")
    print()
