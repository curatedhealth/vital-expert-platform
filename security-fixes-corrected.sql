-- Corrected Security Fixes for VITAL Path Database
...
DECLARE
    ...
    tables_to_check text[] := ARRAY[
        'user_profiles', 'profiles', 'agents', 'workflows', 
        'knowledge_documents', 'audit_logs', 'organizations', 'projects',
        'documents', 'workflow_executions', 'chat_sessions', 'chat_messages',
        'llm_providers', 'llm_usage_logs', 'compliance_records',
        'user_organizations', 'user_sessions', 'performance_metrics',
        'analytics_events', 'alerts', 'metrics'
    ];
BEGIN
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('DROP POLICY IF EXISTS "org_isolation_%s" ON %I', tbl_name, tbl_name);
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_org ON %I(organization_id)', tbl_name, tbl_name);
            RAISE NOTICE 'Created index for table: %', tbl_name;
        END IF;
    END LOOP;
END $$;
