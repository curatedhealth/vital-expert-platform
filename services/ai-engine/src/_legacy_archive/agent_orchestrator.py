...
class AgentOrchestrator:
    ...
    async def process_query_stream(self, request: AgentQueryRequest):
        """
        Process query through appropriate medical agent with STREAMING response.

        This is the streaming version of process_query() that yields tokens
        in real-time as the LLM generates them.

        Yields:
            Dict with event types: token, thinking, sources, done, error
        """
        start_time = datetime.now()

        try:
            logger.info("🧠 Processing STREAMING agent query",
                       agent_type=request.agent_type,
                       query_length=len(request.query))

            # Get or create agent - emit thinking step
            yield {"type": "thinking", "step": "agent_init", "message": "Initializing agent..."}
            agent = await self._get_or_create_agent(request.agent_id, request.agent_type)

            # Get relevant context from RAG - emit thinking step
            yield {"type": "thinking", "step": "rag_retrieval", "message": "Retrieving relevant context from knowledge base..."}
            rag_context = await self._get_rag_context(request)

            # Emit sources from RAG
            sources = self._extract_citations(rag_context)
            if sources:
                yield {"type": "sources", "sources": sources}

            # Stream the agent query - forward all events
            final_response = None
            async for event in self._execute_agent_query_stream(agent, request, rag_context):
                if event["type"] == "done":
                    final_response = event["response"]
                    # Don't yield done yet, we need to do post-processing
                else:
                    yield event

            # Update agent metrics
            if final_response:
                await self._update_agent_metrics(agent["id"], final_response)

                # Log query for audit trail
                if request.user_id and request.organization_id:
                    await self._log_query(request, final_response)
            ...
            logger.error("❌ Streaming query processing failed", error=str(e))
    ...
    async def _execute_agent_query_stream(
        self,
        agent: Dict[str, Any],
        request: AgentQueryRequest,
        rag_context: Dict[str, Any]
        ...
        """
        Execute query through specific agent with STREAMING response.
        Yields tokens as they're generated by the LLM.

        Yields:
            Dict with either:
                - {"type": "token", "content": str} for each token
                - {"type": "thinking", "step": str, "message": str} for thinking steps
                - {"type": "done", "response": AgentQueryResponse} when complete
        ...
        try:
            ...
            # Emit thinking step: Building prompt
            yield {"type": "thinking", "step": "prompt_build", "message": "Building specialized medical prompt..."}
            ...
            # Emit thinking step: Starting LLM
            yield {"type": "thinking", "step": "llm_start", "message": f"Generating response with {self.settings.openai_model}..."}
            ...
            # Emit thinking step: Processing response
            yield {"type": "thinking", "step": "post_process", "message": "Processing response and extracting citations..."}

            # Handle empty response
            response_content = full_response if full_response.strip() else "I was unable to generate a response."
            ...
            final_response = AgentQueryResponse(
                agent_id=agent["id"],
            ...
            logger.error("❌ Agent streaming query failed", error=str(e))
            yield {"type": "error", "message": f"Error: {str(e)}"}
