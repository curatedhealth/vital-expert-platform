...
- Multi-criteria ranking (similarity, domain, level, capabilities)
...
Be precise with domain classification and keyword extraction."""
    ...
    async def _select_agents_legacy(
        ...
        for agent in agents:
            ...
            # Level weighting (fallback to tier for legacy data)
            level = agent.get("agent_level") or agent.get("tier", 2)
            if analysis.complexity == "high" and level == 3:
                score += 0.2
            elif analysis.complexity == "medium" and level == 2:
                score += 0.1
