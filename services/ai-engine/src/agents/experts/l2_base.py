"""
VITAL Path AI Services - VITAL L2 Domain Expert Base

Base class for all L2 Domain Experts in the VITAL hierarchy.
...
class L2DomainExpert(BaseAgent, ABC):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            temperature: LLM temperature (from env/db if None)
            personality_config: Full personality configuration (NEW Phase 2.5)

        Config Resolution: Database → Environment Variables → _L2_DEFAULTS
        ...
        self.model = config.model or _L2_DEFAULTS.model
        self.personality_config = personality_config or {}
        
        # Apply personality temperature if provided, else use L2 env defaults
        effective_temperature = self.personality_config.get(
            'temperature', temperature or _L2_DEFAULTS.temperature
        )
        effective_max_tokens = self.personality_config.get(
        ...
        if llm:
            ...
            try:
                ...
                self.llm = ChatAnthropic(
                    model=self.model,
                    temperature=effective_temperature,  # FROM PERSONALITY or L2_DEFAULTS
                    max_tokens=effective_max_tokens,
                    top_p=self.personality_config.get('top_p', 0.9),
                )
            except ImportError:
                logger.warning(
        ...
        logger.info(
            ...
            personality_slug=self.personality_config.get('slug'),
        )
    ...
    async def spawn_specialist(
        ...
        """
        from modules.ask_expert.agents.specialists import AskExpertL3TaskSpecialist
