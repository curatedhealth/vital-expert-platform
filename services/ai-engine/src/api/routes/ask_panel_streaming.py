...
class SimpleRAGService:
    """
    Minimal RAG service used for local/dev when the full UnifiedRAGService
    cannot be initialized due to missing external services (Pinecone, Neo4j,
    Redis, etc.).

    It implements the `query` interface expected by EnhancedAskPanelWorkflow
    but always returns an empty context with no sources. This still exercises
    the full LangGraph panel logic and LLM calls without requiring vector DBs.
    """
...
async def get_workflow() -> EnhancedAskPanelWorkflow:
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # RAG DISABLED FOR THIS WORKFLOW
        #
        # The full UnifiedRAGService (Pinecone / Neo4j / advanced pipelines)
        # has been a source of initialization errors in this local setup
        # (e.g. missing files / models), which surface as 503s at the panel
        # endpoint.
        #
        # To keep the Python backend and Supabase-based agents working
        # reliably, we always use the minimal SimpleRAGService here. It
        # implements the same interface but returns an empty context so the
        # panel logic and LLM can still run without any vector DBs.
        # ------------------------------------------------------------------
        rag_service: SimpleRAGService = SimpleRAGService()
...
async def list_available_agents(
    ...
    - tier: Filter by agent tier (1, 2, 3)
