"""
Mode 2: Automatic Interactive - Auto Agent Selection + Multi-Turn Conversation

2×2 GOLDEN MATRIX TAXONOMY:
┌─────────────────────┬───────────────────┬───────────────────────────┐
│                     │ MANUAL SELECTION  │ AUTOMATIC SELECTION       │
├─────────────────────┼───────────────────┼───────────────────────────┤
│ INTERACTIVE         │ Mode 1            │ ★ MODE 2 (THIS)           │
...
File: mode2_automatic_interactive.py
Class: Mode2AutomaticInteractiveWorkflow
...
FEATURES:
- Interactive chat-style conversation (like ChatGPT)
...
PRD Specification:
- Selection: AUTOMATIC (System picks best agent via Hybrid RRF)
- Interaction: INTERACTIVE (Multi-turn chat)
- Response Time: 3-5 seconds per turn
- Experts: Auto-selected (can spawn sub-agents)

FRONTEND MAPPING:
- isAutomatic: true (system auto-selects agent)
- isAutonomous: false (interactive, not goal-driven)
...
class Mode2AutomaticInteractiveWorkflow(BaseWorkflow, ToolChainMixin, MemoryIntegrationMixin):
    """
    ...
    def __init__(
        ...
        try:
            ...
            self.cache_manager = None  # Cache is optional
    ...
    async def check_conversation_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        return {
            **state,
            ...
            'nodes_executed': ['check_conversation']
        }
    ...
    async def fresh_conversation_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        return {
            **state,
            ...
            'nodes_executed': ['fresh_conversation']
        }
    ...
    async def direct_execution_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        return {
            **state,
            ...
            'tools_enabled': False,
            'rag_enabled': False,
            'nodes_executed': ['direct_execution']
        }
    ...
    async def handle_save_error_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        return {
            **state,
            ...
            'nodes_executed': ['handle_save_error']
        }
