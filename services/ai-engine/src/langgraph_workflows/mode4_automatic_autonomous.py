"""
Mode 4: Automatic Autonomous - Auto Agent Selection + Goal-Driven Execution
...
├─────────────────────┼───────────────────┼───────────────────────────┤
│ AUTONOMOUS          │ Mode 3            │ ★ MODE 4 (THIS)           │
│ (ReAct/CoT/Goals)   │ User picks agent  │ System picks agent        │
└─────────────────────┴───────────────────┴───────────────────────────┘

File: mode4_automatic_autonomous.py
Class: Mode4AutomaticAutonomousWorkflow
...
class Mode4AutomaticAutonomousWorkflow(BaseWorkflow):
    """
    ...
    async def initialize_hitl_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        return {**state, 'hitl_initialized': hitl_enabled, 'current_node': 'initialize_hitl', 'nodes_executed': ['initialize_hitl']}
    ...
    async def assess_tier_autonomous_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        return {
            **state,
            ...
            'nodes_executed': ['assess_tier_autonomous']
        }
    ...
    async def plan_with_tot_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        if not PATTERNS_AVAILABLE or not self.tot_agent:
            return {**state, 'plan': {'steps': [{'description': state['query'], 'confidence': 0.7}], 'confidence': 0.7}, 'plan_generated': 'fallback', 'nodes_executed': ['plan_with_tot']}
        ...
        try:
            ...
            return {**state, 'plan': plan, 'plan_confidence': plan.get('confidence', 0.0), 'plan_generated': 'tot', 'nodes_executed': ['plan_with_tot']}
        except Exception as e:
            logger.error("ToT planning failed", error=str(e))
            return {**state, 'plan': {'steps': [{'description': state['query'], 'confidence': 0.5}], 'confidence': 0.5}, 'plan_generated': 'error', 'nodes_executed': ['plan_with_tot']}
    ...
    async def request_plan_approval_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        if not self.hitl_service or not state.get('hitl_initialized'):
            return {**state, 'plan_approved': True, 'nodes_executed': ['request_plan_approval']}
        ...
        try:
            ...
            if approval.status == 'rejected':
                ...
                return {**state, 'status': ExecutionStatus.CANCELLED, 'plan_approved': False, 'nodes_executed': ['request_plan_approval']}

            logger.info("Plan approved by user")
            return {**state, 'plan_approved': True, 'nodes_executed': ['request_plan_approval']}
        except Exception as e:
            logger.error("Plan approval failed", error=str(e))
            return {**state, 'plan_approved': False, 'nodes_executed': ['request_plan_approval']}
    ...
    async def execute_with_react_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        if not PATTERNS_AVAILABLE or not self.react_agent:
            return {**state, 'step_results': [], 'pattern_applied': 'none', 'nodes_executed': ['execute_with_react']}
        ...
        try:
            ...
            return {**state, 'agent_response': result.get('response', ''), 'citations': state.get('citations', []) + result.get('citations', []), 'pattern_applied': 'react', 'nodes_executed': ['execute_with_react']}
        except Exception as e:
            logger.error("ReAct execution failed", error=str(e))
            return {**state, 'pattern_applied': 'error', 'nodes_executed': ['execute_with_react']}
    ...
    async def validate_with_constitutional_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        if not PATTERNS_AVAILABLE or not self.constitutional_agent:
            return {**state, 'safety_validated': False, 'nodes_executed': ['validate_with_constitutional']}
        ...
        try:
            ...
            return {**state, 'agent_response': response, 'safety_validated': True, 'safety_score': critique_result.get('safety_score', 0.0), 'nodes_executed': ['validate_with_constitutional']}
        except Exception as e:
    ...
    async def request_decision_approval_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        if not self.hitl_service or not state.get('hitl_initialized'):
            return {**state, 'decision_approved': True, 'nodes_executed': ['request_decision_approval']}
        ...
        try:
            ...
            if approval.status == 'rejected':
                ...
                return {**state, 'requires_revision': True, 'decision_approved': False, 'nodes_executed': ['request_decision_approval']}

            logger.info("Decision approved by user")
            return {**state, 'decision_approved': True, 'nodes_executed': ['request_decision_approval']}
        except Exception as e:
            logger.error("Decision approval failed", error=str(e))
            return {**state, 'decision_approved': False, 'nodes_executed': ['request_decision_approval']}
        ...
        if not session_id:
            ...
            return {
                **state,
                ...
                'nodes_executed': ['load_conversation']
            }
        ...
        try:
            ...
            return {
                **state,
                ...
                'nodes_executed': ['analyze_complexity_domains']
            }
            ...
            if selected_agents and len(selected_agents) > 0:
                ...
                return {
                    **state,
                    ...
                    'nodes_executed': ['select_experts_auto']
                }
    ...
    async def plan_parallel_reasoning_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        - TodoManager for task decomposition per expert
        ...
        try:
            # Create structured reasoning plans for each expert
            expert_reasoning_plans = {}
            expert_tasks = {}  # Structured version for DeepAgents

            for expert_id in selected_agents:
                # Create domain-specific reasoning steps (structured)
                tasks = [
                    {"step": 1, "task": f"Analyze {query[:100]} from expert perspective", "status": "pending", "expert_id": expert_id},
                    {"step": 2, "task": "Gather domain-specific evidence and context via RAG", "status": "pending", "expert_id": expert_id},
                    {"step": 3, "task": "Apply expert knowledge systematically", "status": "pending", "expert_id": expert_id},
                    {"step": 4, "task": "Provide evidence-based recommendations", "status": "pending", "expert_id": expert_id}
                ]

                expert_tasks[expert_id] = tasks
                expert_reasoning_plans[expert_id] = [
                    f"{t['step']}. {t['task']}" for t in tasks
                ]

            # Use DeepAgentsTools TodoManager to persist the multi-expert task plan
            todo_results = []
            if session_id and self.deepagents_tools and len(selected_agents) > 0:
                try:
                    from uuid import UUID
                    session_uuid = UUID(str(session_id)) if isinstance(session_id, str) else session_id

                    # Persist combined task plan (all experts)
                    all_tasks = []
                    for expert_id, tasks in expert_tasks.items():
                        all_tasks.extend(tasks)

                    first_agent_id = selected_agents[0]
                    agent_uuid = UUID(str(first_agent_id)) if isinstance(first_agent_id, str) else first_agent_id

                    todo_result = await self.deepagents_tools.write_todos(
                        session_id=session_uuid,
                        agent_id=agent_uuid,
                        title=f"Mode 4 Multi-Expert Task Plan: {query[:50]}... ({len(selected_agents)} experts)",
                        tasks=all_tasks
                    )
                    todo_results.append(todo_result)
                    logger.info("DeepAgents TodoManager persisted multi-expert task plan",
            ...
            logger.info(
                "Parallel reasoning planned with DeepAgents (Mode 4)",
                experts=len(expert_reasoning_plans),
                todo_persisted=len(todo_results) > 0
            )

            return {
                **state,
                'expert_reasoning_plans': expert_reasoning_plans,
                'expert_reasoning_tasks': expert_tasks,  # Structured version for DeepAgents
                'reasoning_method': 'parallel_chain_of_thought_deepagents',
                'todo_files': [r.get('todo_file') for r in todo_results if r],
                'current_node': 'plan_parallel_reasoning',
                'nodes_executed': ['plan_parallel_reasoning']
            }
    ...
    async def execute_experts_parallel_autonomous_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        try:
            ...
            # OPTIMIZATION: Wait with 60-second timeout (3 experts * 30s + buffer for complex queries)
            ...
            return {
                **state,
                ...
                'nodes_executed': ['execute_experts_parallel_autonomous']
            }
...
"""
            ...
            # OPTIMIZATION: Execute with 30-second timeout per expert (allows for complex RAG + reasoning)
        ...
        if len(agent_responses) == 0:
            return {
                **state,
                ...
                'nodes_executed': ['build_consensus_with_debate']
            }
    ...
    async def check_expert_rotation_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        return {
            **state,
            ...
            'nodes_executed': ['check_expert_rotation']
        }
            ...
            if new_expert_id and new_expert_id not in current_agents:
                ...
                return {
                    **state,
                    ...
                    'nodes_executed': ['bring_new_expert']
                }
    ...
    async def continue_with_current_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        return {
            **state,
            ...
            'nodes_executed': ['continue_with_current']
        }
    ...
    async def save_conversation_node(self, state: UnifiedWorkflowState) -> UnifiedWorkflowState:
        ...
        if not session_id:
            ...
            return {**state, 'current_node': 'save_conversation', 'nodes_executed': ['save_conversation']}
        ...
        try:
            ...
            return {
                **state,
                ...
                'nodes_executed': ['save_conversation']
            }
...
"""
                ...
                return {
                    **state,
                    ...
                    'nodes_executed': ['validate_human_review']
                }
            else:
                return {
                    **state,
