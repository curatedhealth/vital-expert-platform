"""
VITAL Path AI Services - Panel Configuration

Centralized configuration for Panel workflows.
Removes all hardcoded values and provides environment-based settings.

Usage:
    from services.panel import PanelConfig, get_panel_config
...
@lru_cache()
def get_panel_config() -> PanelConfig:
    """
    Get Ask Panel configuration (cached singleton).

    Returns:
        PanelConfig instance with values from environment
    """
    return PanelConfig()
