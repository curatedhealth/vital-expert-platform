...
export class SupabaseRAGService {
  private const supabase = upabase;
  ...
  async searchKnowledgeSources(
    ...
    if (domain) {
      const query = uery.eq('domain', domain);
    }

    if (category) {
      const query = uery.eq('category', category);
    }
  ...
  async addDocumentChunks(chunks: {
    ...
    const chunkData = hunks.map(chunk => ({
  ...
  async searchWithContext(
    ...
    const results = wait this.search({
      ...query,
  ...
  async enhancedSearch(question: string, options: {
    ...
    try {
      ...
      const results = wait this.searchKnowledge({
        text: question,
...
export const __supabaseRAGService = ew SupabaseRAGService();