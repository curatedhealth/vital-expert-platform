-- ============================================================================
-- Share Organizational Structure Data
-- ============================================================================
-- Shares org_functions, org_departments, org_roles from VITAL Platform to:
-- - Pharmaceuticals
...
DECLARE
  ...
  v_tenant_id UUID;
  v_tenant_name TEXT;
  v_functions_added INT := 0;
  v_departments_added INT := 0;
  v_roles_added INT := 0;
  v_total_functions INT := 0;
  v_total_departments INT := 0;
  v_total_roles INT := 0;
  v_record RECORD;
...
BEGIN
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '🔄 ORGANIZATIONAL STRUCTURE DATA SHARING';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  ...
  -- PHASE 1: Assign all existing org structure data to VITAL Platform
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '📋 PHASE 1: Assigning to VITAL Platform';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';

  UPDATE public.org_functions
  SET tenant_id = v_platform_tenant_id
  WHERE tenant_id != v_platform_tenant_id OR tenant_id IS NULL;

  GET DIAGNOSTICS v_total_functions = ROW_COUNT;
  RAISE NOTICE '✅ Assigned % functions to VITAL Platform', v_total_functions;

  UPDATE public.org_departments
  SET tenant_id = v_platform_tenant_id
  WHERE tenant_id != v_platform_tenant_id OR tenant_id IS NULL;

  GET DIAGNOSTICS v_total_departments = ROW_COUNT;
  RAISE NOTICE '✅ Assigned % departments to VITAL Platform', v_total_departments;

  UPDATE public.org_roles
  SET tenant_id = v_platform_tenant_id
  WHERE tenant_id != v_platform_tenant_id OR tenant_id IS NULL;

  GET DIAGNOSTICS v_total_roles = ROW_COUNT;
  RAISE NOTICE '✅ Assigned % roles to VITAL Platform', v_total_roles;
  RAISE NOTICE '';

  -- PHASE 2: Share with each tenant
  ...
  LOOP
    ...
    FOR v_record IN
      SELECT * FROM public.org_functions WHERE tenant_id = v_platform_tenant_id
    LOOP
      ...
      v_functions_added := v_functions_added + 1;
    END LOOP;

    RAISE NOTICE '   ✅ Added % functions', v_functions_added;
    v_total_functions := v_total_functions + v_functions_added;
    v_functions_added := 0;

    -- 2. Copy Departments (with function_id mapping)
    RAISE NOTICE '🏬 Copying departments...';
    DELETE FROM public.org_departments WHERE tenant_id = v_tenant_id;

    FOR v_record IN
      SELECT * FROM public.org_departments WHERE tenant_id = v_platform_tenant_id
    LOOP
      ...
      v_departments_added := v_departments_added + 1;
    END LOOP;

    RAISE NOTICE '   ✅ Added % departments', v_departments_added;
    v_total_departments := v_total_departments + v_departments_added;
    v_departments_added := 0;

    -- 3. Copy Roles (with function_id and department_id mapping)
    RAISE NOTICE '👔 Copying roles...';
    DELETE FROM public.org_roles WHERE tenant_id = v_tenant_id;

    FOR v_record IN
      SELECT * FROM public.org_roles WHERE tenant_id = v_platform_tenant_id
    LOOP
      -- Map function_id if it exists
      v_new_function_id := NULL;
      IF v_record.function_id IS NOT NULL THEN
    ...
    RAISE NOTICE '   ✅ Added % roles', v_roles_added;
    v_total_roles := v_total_roles + v_roles_added;
    v_roles_added := 0;
  ...
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE 'Total functions shared:   %', v_total_functions;
  RAISE NOTICE 'Total departments shared: %', v_total_departments;
  RAISE NOTICE 'Total roles shared:       %', v_total_roles;
  RAISE NOTICE '';
  RAISE NOTICE 'Each tenant now has:';

  -- Get actual counts from platform
  SELECT COUNT(*) INTO v_total_functions FROM public.org_functions WHERE tenant_id = v_platform_tenant_id;
  SELECT COUNT(*) INTO v_total_departments FROM public.org_departments WHERE tenant_id = v_platform_tenant_id;
  SELECT COUNT(*) INTO v_total_roles FROM public.org_roles WHERE tenant_id = v_platform_tenant_id;

  RAISE NOTICE '  - % functions', v_total_functions;
  RAISE NOTICE '  - % departments', v_total_departments;
  RAISE NOTICE '  - % roles', v_total_roles;
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
