...
interface HIPAAReportProps {
  report: ComplianceReport;
  onRefresh: () => void;
  isLoading: boolean;
}

export default function HIPAAReport({ report, onRefresh, isLoading }: HIPAAReportProps) {
        ...
        <Alert variant={report.status === 'non_compliant' ? 'destructive' : 'default'}>
          ...
          <AlertDescription>
            {report.status === 'non_compliant' 
              ? 'HIPAA compliance is not met. Critical issues require immediate attention.'
              : 'HIPAA compliance is partially met. Some issues need to be addressed.'
            }
          </AlertDescription>
        </Alert>
      )}
        ...
        <CardContent>
          {report.findings.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              No compliance findings. All HIPAA requirements are met.
            </div>
          ) : (
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-muted-foreground">PHI Access Logs:</span>
              <span className="ml-2 font-medium">{report.metadata.phiAccessLogs || 0}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Encryption Status Checks:</span>
              <span className="ml-2 font-medium">{report.metadata.encryptionStatus || 0}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Access Controls:</span>
              <span className="ml-2 font-medium">{report.metadata.accessControls || 0}</span>
            </div>
            <div>
