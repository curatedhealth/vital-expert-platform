...
interface SOC2ReportProps {
  report: ComplianceReport;
  onRefresh: () => void;
  isLoading: boolean;
}

export default function SOC2Report({ report, onRefresh, isLoading }: SOC2ReportProps) {
        ...
        <Alert variant={report.status === 'non_compliant' ? 'destructive' : 'default'}>
          ...
          <AlertDescription>
            {report.status === 'non_compliant' 
              ? 'SOC2 compliance is not met. Critical issues require immediate attention.'
              : 'SOC2 compliance is partially met. Some issues need to be addressed.'
            }
          </AlertDescription>
        </Alert>
      )}
        ...
        <CardContent>
          {report.findings.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              No compliance findings. All SOC2 requirements are met.
            </div>
          ) : (
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <span className="ml-2 font-medium">{report.metadata.totalUsers || 0}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Inactive Users:</span>
              <span className="ml-2 font-medium">{report.metadata.inactiveUsers || 0}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Recent Changes:</span>
              <span className="ml-2 font-medium">{report.metadata.recentChanges || 0}</span>
            </div>
            <div>
