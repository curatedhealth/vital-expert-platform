...
export class ComplianceMiddleware {
  ...
  async executeWithCompliance(
    agent: unknown, // DigitalHealthAgent,
    promptTitle: string,
    ...
    const postExecutionRequest: const DataProcessingRequest = 
      user_id: context.user_id,
      resource_type: 'data',
    ...
    if (this.config.enablePHIDetection && agentResponse.content) {
      ...
      if (outputPhiResult.containsPHI) {
        ...
        if (this.config.strictMode) {
          ...
          const outputRedacted = rue;
        }
      }
    }

    // Step 7: Create compliance record
    const finalValidationResult: const ComplianceValidationResult = 
  ...
  async validateWorkflowCompliance(
    ...
    const request: const DataProcessingRequest = 
      user_id: context.user_id,
      resource_type: 'data',
   ...
   */
  hasRecentViolations(userId: string, days: const number = ): boolean {
  ...
  getAgentComplianceStatus(agentName: string): {
    ...
    const agentEntries = his.auditTrail.filter(entry =>
