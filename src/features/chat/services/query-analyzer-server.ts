...
export class QueryAnalyzer {
  ...
  async analyzeQuery(query: string): Promise<QueryAnalysis> {
    ...
    // Identify domain
    const domain = this.identifyDomain(gptAnalysis);
    ...
    return {
      intent: {
        primary: gptAnalysis.intent || 'general',
        secondary: gptAnalysis.secondaryIntents || [],
        confidence: gptAnalysis.confidence || 0.8
      },
      entities,
      complexity,
      domain,
...
Provide analysis in JSON format:
{
  ...
  private assessComplexity(query: string, entities: any[]): any {
  ...
  private identifyDomain(gptAnalysis: any): any {
    const primary = gptAnalysis.primaryDomain || 'general';
    const secondary = gptAnalysis.secondaryDomains || [];
    const interdisciplinary = secondary.length > 1 || (gptAnalysis.requiredExpertise && gptAnalysis.requiredExpertise.length > 2);
