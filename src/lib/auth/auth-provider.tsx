...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const createAuthUser = useCallback(async (supabaseUser: SupabaseUser): Promise<AuthUser> => {
    try {
      ...
      if (profileError) {
        ...
        const defaultRole: UserRole = AUTH_CONFIG.superAdminEmails.includes(supabaseUser.email || '') 
          ? 'super_admin' 
          : 'user';
  ...
  const initializeAuth = useCallback(async () => {
    try {
      ...
      const friendlyMessage = authErrorRecovery.getUserFriendlyMessage(error);
  ...
  useEffect(() => {
    ...
    const unsubscribeSync = sessionSync.subscribe(() => {
