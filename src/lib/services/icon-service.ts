...
export class IconService {
  ...
  async getIconsByCategory(category: Icon['category']): Promise<Icon[]> {
    try {
      const response = await this.fetchAPI(`?category=${category}`);
      return response.icons || [];
    } catch (error) {
  ...
  async getIconsByCategories(categories: Icon['category'][]): Promise<Icon[]> {
    try {
      const response = await this.fetchAPI(`?categories=${categories.join(',')}`);
      return response.icons || [];
    } catch (error) {
  ...
  async searchIcons(query: string, category?: Icon['category']): Promise<Icon[]> {
    try {
      ...
      const response = await this.fetchAPI(`?${params.toString()}`);
      return response.icons || [];
    } catch (error) {
  ...
  async getIconByName(name: string): Promise<Icon | null> {
    try {
      const response = await this.fetchAPI(`?search=${name}`);
