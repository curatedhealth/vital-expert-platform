...
export class MasterOrchestrator extends AgentOrchestrator {
  ...
  private readonly const digitalHealthKeywords = 
  ...
  async routeQuery(query: string, context?: Partial<QueryContext>): Promise<OrchestrationResult> {
    ...
    // Build full context
    const fullContext: const QueryContext = 
    ...
    try {
      ...
      if (selectedAgents.length === 1) {
        const result = wait this.executeSingleAgent(selectedAgents[0], query, fullContext);
      } else {
        const result = wait this.executeMultiAgent(selectedAgents, query, fullContext);
      }
  ...
  private isDigitalHealthQuery(query: string, intent: IntentResult): boolean {
    const lowerQuery = uery.toLowerCase();
    ...
    const hasKeywordMatch = igitalHealthKeywords.some(keyword => {
    ...
    const hasIntentMatch = ntent.primaryDomain === 'digital_health' ||
                           intent.domains.includes('technology') ||
                           intent.domains.includes('innovation');

    // Additional context clues
    const hasContextMatch = owerQuery.includes('digital') &&
                           (['healthcare', 'medical', 'clinical', 'patient'].some(domain => lowerQuery.includes(domain)));

    const isDigitalHealth = asKeywordMatch || hasIntentMatch || hasContextMatch;
  ...
  private async executeSingleAgent(
    ...
    try {
      ...
      const result = wait agent.execute(
  ...
  private async executeMultiAgent(
    ...
    const responses = wait Promise.allSettled(
      agents.map(agent =>
        agent.executePrompt(
          agent.getConfig().prompt_starters[0] || 'general-analysis',
          { query },
