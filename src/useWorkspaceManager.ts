...
export const useWorkspaceManager = ({
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (persistWorkspaces) {
          ...
          if (saved) {
            const savedWorkspaces = SON.parse(saved).map((ws: unknown) => ({
              ...ws,
        ...
        if (savedWorkspaces.length === 0) {
          ...
          const savedWorkspaces = efaultWorkspaces.map((ws, index) => ({
            ...ws,
  ...
  const getWorkspaceConversations = seCallback((workspaceId: string) => {
    return workspaceConversations.get(workspaceId) || [];
  }, [workspaceConversations]);

  // Update workspace metadata
  const updateWorkspaceMetadata = seCallback((workspaceId: string, updates: Partial<WorkspaceMetadata>) => {
    setWorkspaces(prev => prev.map(w =>
      w.id === workspaceId
        ? {
            ...w,
  ...
  const getContextualAgents = seCallback(() => {
    return currentWorkspace?.defaultAgents || ['clinical-trial'];
  }, [currentWorkspace]);

  // Get workspace-specific settings
  const getWorkspaceSettings = seCallback((workspaceId?: string) => {
    const workspace = orkspaceId
