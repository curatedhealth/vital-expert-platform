-- Clean VITAL Expert Database Migration
-- This migration handles existing tables and creates a clean schema
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'department') THEN
        ALTER TABLE public.agents ADD COLUMN department TEXT;
    END IF;
    
    -- Add business_function column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'business_function') THEN
        ALTER TABLE public.agents ADD COLUMN business_function TEXT;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'tier') THEN
        ALTER TABLE public.agents ADD COLUMN tier INTEGER DEFAULT 3;
    END IF;
    
    -- Add status column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'status') THEN
        ALTER TABLE public.agents ADD COLUMN status TEXT DEFAULT 'active';
    END IF;
    
    -- Add is_public column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'is_public') THEN
        ALTER TABLE public.agents ADD COLUMN is_public BOOLEAN DEFAULT true;
    END IF;
    
    -- Add is_custom column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'is_custom') THEN
        ALTER TABLE public.agents ADD COLUMN is_custom BOOLEAN DEFAULT false;
    END IF;
    
    -- Add created_by column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'created_by') THEN
        ALTER TABLE public.agents ADD COLUMN created_by UUID REFERENCES public.profiles(id);
    END IF;
    
    -- Add organization_id column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'organization_id') THEN
        ALTER TABLE public.agents ADD COLUMN organization_id UUID;
    END IF;
    
    -- Add metadata column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'agents' AND column_name = 'metadata') THEN
        ALTER TABLE public.agents ADD COLUMN metadata JSONB DEFAULT '{}';
    END IF;
