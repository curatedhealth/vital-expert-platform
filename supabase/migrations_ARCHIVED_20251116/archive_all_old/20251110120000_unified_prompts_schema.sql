-- ============================================================================
-- UNIFIED PROMPTS™ SCHEMA - Single Source of Truth
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS prompts (
  ...
  CONSTRAINT prompts_tenant_unique_id_key UNIQUE (tenant_id, unique_id)
);
...
CREATE TABLE IF NOT EXISTS prompt_suites (
  ...
  CONSTRAINT prompt_suites_tenant_unique_id_key UNIQUE (tenant_id, unique_id)
);
...
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'dh_prompt')
...
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'dh_prompt_suite')
...
-- ============================================================================
-- 5. SEED 10 PRISM SUITES (If table is empty)
-- ============================================================================
...
CREATE POLICY "Users can view prompts in their tenant"
  ...
  USING (
    ...
    OR tenant_id IS NULL  -- Public prompts
  );

CREATE POLICY "Users can insert prompts in their tenant"
...
CREATE POLICY "Users can update prompts in their tenant"
...
CREATE POLICY "Users can delete prompts in their tenant"
...
-- Prompt Suites Policies
CREATE POLICY "Users can view prompt suites in their tenant"
  ON prompt_suites FOR SELECT
  USING (
    tenant_id IN (
      SELECT tenant_id FROM user_profiles WHERE user_id = auth.uid()
    )
    OR tenant_id IS NULL  -- Public suites
  );

CREATE POLICY "Admins can manage prompt suites"
